...
class Checker:
    ...
    def check(self, path):
        ...
        if not isinstance(path.posix.stdin, bisa.storage.file.SimPacketsStream):
...
class TestScanf(unittest.TestCase):
    def test_scanf(self):
        ...
        b = bisa.Project(test_bin, auto_load_libs=False)
    ...
    def test_scanf_simfile_string(self):
        ...
        stdin = bisa.SimFile("/dev/stdin", content=blist)

        s = b.factory.entry_state(stdin=stdin)
        simfd = s.posix.get_fd(0)

        assert isinstance(simfd.read_storage, bisa.SimFile)
