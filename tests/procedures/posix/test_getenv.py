...
class TestRunEcho(unittest.TestCase):
    ...
    def test_run_getenv_with_env(self):
        env = {"PATH": "/home/bisa/", "TEST_ENV1": "this_is_a_string!", "JAVA_HOME": "jdk-install-dir"}
        TEST_name = ["TEST_ENV1", "TEST_ENV2"]
        p = bisa.Project(os.path.join(test_location, "x86_64", "test_getenv"))
    ...
    def test_run_getenv_with_symbolic_env(self):
        ...
        env = {"PATH": "/home/bisa/", "TEST_ENV1": flag, "JAVA_HOME": "jdk-install-dir"}
        p = bisa.Project(os.path.join(test_location, "x86_64", "test_getenv"))
        s = p.factory.entry_state(env=env)
        simgr = p.factory.simulation_manager(s)
        simgr.explore()
