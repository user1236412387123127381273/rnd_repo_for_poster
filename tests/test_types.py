...
from bisa.utils.library import convert_cproto_to_py, convert_cppproto_to_py
...
class TestTypes(unittest.TestCase):
    ...
    def test_struct_deduplication(self):
        bisa.types.register_types(bisa.types.parse_type("struct ahdr { int a ;}"))
        bisa.types.register_types(bisa.types.parse_type("struct bhdr { int b ;}"))
        bisa.types.register_types(bisa.types.parse_type("struct chdr { int c ;}"))
        dhdr = bisa.types.parse_type("struct dhdr { struct ahdr a; struct bhdr b; struct chdr c;}")
        assert isinstance(dhdr, SimStruct)
        assert isinstance(dhdr.fields["a"], SimStruct)
        assert dhdr.fields["a"].fields

    def test_parse_type(self):
        int_ptr = bisa.types.parse_type("int *")
        assert isinstance(int_ptr, SimTypePointer)
        assert isinstance(int_ptr.pts_to, SimTypeInt)

        char_ptr = bisa.types.parse_type("char *c")
        assert isinstance(char_ptr, SimTypePointer)
        assert isinstance(char_ptr.pts_to, SimTypeChar)

        struct_parse_type = bisa.types.parse_type("struct parse_type { char c; float f; }")
        ...
        union_dcba = bisa.types.parse_type("union dcba { double d; long long int lli; }")
        ...
        struct_llist = bisa.types.parse_type("struct llist { int data; struct llist * next; }")
        ...
        func_ptr = bisa.types.parse_type("double (*) (int, float)")
    ...
    def test_parse_type_no_basic_types(self):
        time_t = bisa.types.parse_type("time_t")
        assert isinstance(time_t, SimTypeLong)

        byte = bisa.types.parse_type("byte")
    ...
    def test_self_referential_struct_or_union(self):
        ...
        union_heap = bisa.types.parse_type("union heap { int data; union heap *forward; }")
    ...
    def test_union_struct_referencing_each_other(self):
        bisa.types.register_types(bisa.types.parse_type("struct a"))
        bisa.types.register_types(bisa.types.parse_type("struct b"))
        a = bisa.types.parse_type("struct a { struct b *b_ptr; }")
        b = bisa.types.parse_type("struct b { struct a *a_ptr; }")
        ...
        bisa.types.register_types(bisa.types.parse_type("union a"))
        bisa.types.register_types(bisa.types.parse_type("union b"))
        a = bisa.types.parse_type("union a { union b *b_ptr; }")
        b = bisa.types.parse_type("union b { union a *a_ptr; }")
    ...
    def test_top_type(self):
        bisa.types.register_types({"undefined": SimTypeTop()})
        fdef: dict[str, SimTypeFunction] = bisa.types.parse_defns("undefined f(undefined param_1, int param_2);")
        sig = fdef["f"]
        assert sig.args == (SimTypeTop(), SimTypeInt())

    def test_arg_names(self):
        bisa.types.register_types({"undefined": SimTypeTop()})
        fdef: dict[str, SimTypeFunction] = bisa.types.parse_defns("int f(int param_1, int param_2);")
        sig = fdef["f"]
        ...
        fdef: dict[str, SimTypeFunction] = bisa.types.parse_defns("int f(int param1, int);")
        sig = fdef["f"]
        assert sig.arg_names == ("param1", None)

        fdef: dict[str, SimTypeFunction] = bisa.types.parse_defns("int f();")
        sig = fdef["f"]
        assert sig.arg_names == ()

    def test_varargs(self):
        fdef = bisa.types.parse_defns("int printf(const char *fmt, ...);")
        sig = fdef["printf"]
    ...
    def test_forward_declaration_typedef_struct(self):
        _, extra_types = bisa.types.parse_file("typedef struct _A A; struct _A {int a;int b;};")
    ...
    def test_forward_declaration_typedef_union(self):
        _, extra_types = bisa.types.parse_file("typedef union _A A; union _A {int a;int b;};")
    ...
    def test_dereference_type_anonymous_struct(self):
        bisa.procedures.definitions.load_win32_type_collections()
        variant_type = bisa.SIM_TYPE_COLLECTIONS["win32"].get("VARIANT")
        ...
        t = dereference_simtype(variant_type, [bisa.SIM_TYPE_COLLECTIONS["win32"]]).with_arch(archinfo.ArchX86())
