...
class TestVault(unittest.TestCase):
    ...
    def test_vault_noidentity_VaultDir(self):
        self._do_vault_noidentity(bisa.vaults.VaultDir)

    def test_vault_noidentity_VaultShelf(self):
        self._do_vault_noidentity(bisa.vaults.VaultShelf)

    def test_vault_noidentity_VaultDict(self):
        self._do_vault_noidentity(bisa.vaults.VaultDict)

    def test_vault_noidentity_VaultDirShelf(self):
        self._do_vault_noidentity(bisa.vaults.VaultDirShelf)

    def test_vault_identity_VaultDir(self):
        self._do_vault_identity(bisa.vaults.VaultDir)

    def test_vault_identity_VaultShelf(self):
        self._do_vault_identity(bisa.vaults.VaultShelf)

    def test_vault_identity_VaultDict(self):
        self._do_vault_identity(bisa.vaults.VaultDict)
    ...
    def test_vault_identity_VaultDirShelf(self):
        ...
        self._do_vault_identity(bisa.vaults.VaultDirShelf)

    def test_ast_vault_do_ast_vault_VaultDir(self):
        self._do_ast_vault(bisa.vaults.VaultDir)

    def test_ast_vault_do_ast_vault_VaultShelf(self):
        self._do_ast_vault(bisa.vaults.VaultShelf)

    def test_ast_vault_do_ast_vault_VaultDict(self):
        self._do_ast_vault(bisa.vaults.VaultDict)

    @unittest.expectedFailure
    def test_ast_vault_VaultDirShelf(self):
        # VaultDirShelf does not guarantee identity equivalence due to the absence of caching
        self._do_ast_vault(bisa.vaults.VaultDirShelf)

    def test_project(self):
        v = bisa.vaults.VaultDir()
        p = bisa.Project(os.path.join(bin_location, "tests", "x86_64", "fauxware"), auto_load_libs=False)
        ps = v.store(p)
        pp = v.load(ps)
        assert p is pp
