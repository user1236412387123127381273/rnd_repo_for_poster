...
import bisa
from bisa.bisadb import BISADB
...
class TestDb(unittest.TestCase):
    def test_bisadb_fauxware(self):
        bin_path = os.path.join(test_location, "x86_64", "fauxware")

        proj = bisa.Project(bin_path, auto_load_libs=False)
        cfg: bisa.analyses.CFGFast = proj.analyses.CFGFast(data_references=True, cross_references=True, normalize=True)
    ...
    def test_bisadb_open_multiple_times(self):
        bin_path = os.path.join(test_location, "x86_64", "fauxware")

        proj = bisa.Project(bin_path, auto_load_libs=False)
        _: bisa.analyses.CFGFast = proj.analyses.CFGFast(data_references=True, cross_references=True, normalize=True)
        proj.kb.comments[proj.entry] = "Entry point"
    ...
    def test_bisadb_save_multiple_times(self):
        bin_path = os.path.join(test_location, "x86_64", "fauxware")
    ...
    def test_bisadb_save_without_binary_existence(self):
        bin_path = os.path.join(test_location, "x86_64", "fauxware")
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with tempfile.TemporaryDirectory() as td0:
                ...
                proj = bisa.Project(tmp_path, auto_load_libs=False)

                BISADB(proj).dump(db_file)

                del proj
                os.remove(tmp_path)

            # now that the binary file no longer exists, we should be able to open the bisa DB and save it without
            # raising exceptions.

            proj = BISADB().load(db_file)
            try:
                os.remove(db_file)
