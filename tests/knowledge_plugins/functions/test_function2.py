...
class TestFunction(unittest.TestCase):
    def test_function_serialization(self):
        ...
        f = bisa.knowledge_plugins.Function.parse(s)
    ...
    def test_function_definition_application(self):
        p = bisa.Project(os.path.join(test_location, "x86_64", "fauxware"), auto_load_libs=False)
        cfg = p.analyses.CFG()
        func_main: bisa.knowledge_plugins.Function = cfg.kb.functions["main"]
        ...
        assert func_main.prototype.args == (
            bisa.types.SimTypeInt().with_arch(p.arch),
            bisa.types.SimTypePointer(bisa.types.SimTypePointer(bisa.types.SimTypeChar()).with_arch(p.arch)).with_arch(
                p.arch
            ),
        )
        # Check that the default calling convention of the architecture was applied
        assert isinstance(func_main.calling_convention, bisa.calling_conventions.DefaultCC[p.arch.name]["Linux"])
