...
import bisa
from bisa.sim_type import SimTypePointer, SimTypeFunction, SimTypeChar, SimTypeInt, parse_defns
...
class TestCallable(unittest.TestCase):
    def run_fauxware(self, arch):
        ...
        p = bisa.Project(os.path.join(test_location, arch, "fauxware"))
        ...
        self.assertRaises(BISACallableMultistateError, authenticate, "asdf", "NOSNEAKY")
    ...
    def run_manysum(self, arch):
        ...
        p = bisa.Project(os.path.join(test_location, arch, "manysum"))
        inttype = SimTypeInt()
    ...
    def run_manyfloatsum(self, arch):
        ...
        p = bisa.Project(os.path.join(test_location, arch, "manyfloatsum"))
        for function in (
            "sum_floats",
            "sum_combo",
            "sum_segregated",
    ...
    def test_setup_callsite(self):
        p = bisa.load_shellcode(b"b", arch=archinfo.ArchX86())
    ...
    def test_factory_callable_function_arg(self):
        arch = "x86_64"
        addr = addresses_fauxware[arch]
        p = bisa.Project(os.path.join(test_location, arch, "fauxware"), auto_load_libs=False)
        p.analyses.CFG()
        charstar = SimTypePointer(SimTypeChar())
        prototype = SimTypeFunction((charstar, charstar), SimTypeInt(False))
