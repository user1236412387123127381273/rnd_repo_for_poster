...
class TestRelro(unittest.TestCase):
    def _run_fauxware_relro(self, arch):
        p = bisa.Project(os.path.join(test_location, arch, "fauxware"), use_sim_procedures=False)
        s = p.factory.full_init_state(add_options={bisa.options.STRICT_PAGE_ACCESS})
