...
class TestMemcmpDefiniteSize(unittest.TestCase):
    ...
    def test_memcmp_strlen_simprocedure_interaction(self):
        ...
        p = bisa.Project(bin_path, auto_load_libs=True)  # this binary requires the loading of libstdc++.so.6
        argv1 = claripy.Concat(*[claripy.BVS(f"argv{i}", 8) for i in range(48)])

        state = p.factory.full_init_state(args=[bin_path, argv1], add_options=bisa.sim_options.unicorn)
