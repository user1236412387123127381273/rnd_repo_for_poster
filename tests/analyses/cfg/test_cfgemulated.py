...
l = logging.getLogger("bisa.tests.test_cfgemulated")
...
class TestCfgemulate(unittest.TestCase):
    def compare_cfg(self, standard, g, function_list):
        ...
        for src, dst in graph.edges():
            ...
            # Edge doesn't exist in our CFG
            l.error(
                "Edge (%s-0x%x, %s-0x%x) only exists in bisa's CFG.",
                get_function_name(src),
                src,
                get_function_name(dst),
                dst,
    ...
    def test_not_returning(self):
        ...
        proj = bisa.Project(binary_path, use_sim_procedures=True, load_options={"auto_load_libs": False})
    ...
    def test_cfg_6(self):
        ...
        proj = bisa.Project(binary_path, use_sim_procedures=True, page_size=1, auto_load_libs=False)
    ...
    def test_loop_unrolling(self):
        ...
        p = bisa.Project(binary_path, auto_load_libs=True)
    ...
    def test_thumb_mode(self):
        ...
        p = bisa.Project(binary_path, auto_load_libs=False)
        cfg = p.analyses.CFGEmulated(fail_fast=True)
    ...
    def test_fakeret_edges_0(self):
        ...
        # Ref: https://github.com/bisa/bisa/issues/72
        ...
        plt_entry_0 = cfg.model.get_predecessors(putchar_cfgnodes[0])
        ...
        preds_1 = cfg.model.get_predecessors(plt_entry_1)
    ...
    def test_string_references(self):
        ...
        b = bisa.Project(binary_path, load_options={"auto_load_libs": False})
    ...
    def test_armel_final_missing_block(self):
        ...
        b = bisa.Project(binary_path, auto_load_libs=False)
        cfg = b.analyses.CFGEmulated(fail_fast=True)
    ...
    def test_armel_final_missing_block_b(self):
        ...
        b = bisa.Project(binary_path, arch="ARMEL", auto_load_libs=False)
    ...
    def test_cfg_switches(self):
        # logging.getLogger('bisa.analyses.cfg.cfg_fast').setLevel(logging.INFO)
        # logging.getLogger('bisa.analyses.cfg.indirect_jump_resolvers.jumptable').setLevel(logging.DEBUG)
        ...
        for arch in arches:
            ...
            proj = bisa.Project(path, load_options={"auto_load_libs": False})
    ...
    class CFGEmulatedAborted(bisa.analyses.cfg.cfg_emulated.CFGEmulated):  # pylint:disable=abstract-method
        """
    ...
    def test_abort_and_resume(self):
        bisa.analyses.AnalysesHub.register_default("CFGEmulatedAborted", self.CFGEmulatedAborted)
    ...
    def test_base_graph(self):
        ...
        target_function_cfg_emulated = proj.analyses.CFGEmulated(
            keep_state=True,
            state_add_options=bisa.options.refs,
