...
class TestCfgRustGotResolution(unittest.TestCase):
    ...
    def test_rust_got_resolution(self):
        ...
        # This tests whether bisa can resolve indirect function calls to GOT entries in Rust binaries.

        path = os.path.join(test_location, "x86_64", "rust_hello_world")
        p = bisa.Project(path, auto_load_libs=False)

        cfg = p.analyses.CFGFast(resolve_indirect_jumps=True)

        # bisa should be able to resolve the indirect call in main function
