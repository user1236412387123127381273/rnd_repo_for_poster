...
import bisa
from bisa.analyses import CFGFast
from bisa.knowledge_plugins.cfg import MemoryData, MemoryDataSort

if TYPE_CHECKING:
    from bisa.knowledge_plugins.cfg import CFGModel
    from bisa.knowledge_plugins.functions import Function, FunctionManager
...
def apply_patches(proj: bisa.Project, patches: list[tuple[int, str]]):
...
class TestCfgCombination(unittest.TestCase):
    ...
    def test_cfgfast_combine_with_full_model(self):
        ...
        proj = bisa.Project(FAUXWARE_PATH, auto_load_libs=False)
...
class TestCfgReclassification(unittest.TestCase):
    ...
    def test_cfgfast_preclassify_data_as_code(self):
        ...
        proj = bisa.load_shellcode(code, "AMD64")
...
class TestCfgPatching(unittest.TestCase):
    ...
    def _test_patch(self, patches: Sequence[tuple[int, str]]):
        ...
        proj = bisa.Project(unpatched_binary_path, auto_load_libs=False)
        ...
        with tempfile.NamedTemporaryFile(prefix="fauxware-patched-", delete=False) as f:
            ...
            f.close()
            prepatched_proj = bisa.Project(f.name, auto_load_libs=False)
...
if __name__ == "__main__":
    logging.basicConfig()
    log.setLevel(logging.DEBUG)
    logging.getLogger("bisa.analyses.cfg.cfg_fast").setLevel(logging.DEBUG)
    logging.getLogger("bisa.analyses.cfg.cfg_base").setLevel(logging.DEBUG)
    unittest.main()
