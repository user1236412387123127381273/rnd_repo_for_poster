...
import archinfo
import bisa
from bisa.utils.segment_list import SegmentList
from bisa.knowledge_plugins.cfg import CFGNode, CFGModel, MemoryDataSort
from bisa.analyses.cfg.indirect_jump_resolvers import mips_elf_fast

from tests.common import bin_location, slow_test

l = logging.getLogger("bisa.tests.test_cfgfast")
...
class TestCfgfast(unittest.TestCase):
    ...
    def test_cfg_0_pe(self):
        functions = {
            # 0x40150a,  # currently bisa identifies 0x40150e due to the way _func_addrs_from_prologues() is
    ...
    def test_arm_function_merge(self):
        ...
        proj = bisa.Project(path, auto_load_libs=False)
    ...
    def test_cfg_about_time(self):
        ...
        proj = bisa.Project(os.path.join(test_location, "x86_64", "about_time"), auto_load_libs=False)
    ...
    def test_serialization_cfgfast(self):
        path = os.path.join(test_location, "x86_64", "fauxware")
        proj1 = bisa.Project(path, auto_load_libs=False)
        proj2 = bisa.Project(path, auto_load_libs=False)
    ...
    def test_cfg_0_pe_msvc_debug_nocc(self):
        ...
        proj = bisa.Project(os.path.join(test_location, "x86_64", filename), auto_load_libs=False)
        cfg = proj.analyses.CFGFast()
    ...
    def test_function_leading_blocks_merging(self):
        ...
        path = os.path.join(test_location, "armel", "Nucleo_read_hyperterminal-stripped.elf")
        proj = bisa.Project(path, arch=archinfo.ArchARMCortexM(), auto_load_libs=False)
    ...
    def test_load_from_shellcode(self):
        proj = bisa.load_shellcode("loop: dec ecx; jnz loop; ret", "x86")
        cfg = proj.analyses.CFGFast()
