...
class TestReassembler(unittest.TestCase):
    def test_data_reference_collection_in_add(self):
        ...
        # Fixed in https://github.com/bisa/pyvex/pull/192

        p = bisa.Project(os.path.join(test_location, "x86_64", "df_gcc_-O1"), auto_load_libs=False)
    ...
    def test_ln_gcc_O2(self):
        ...
        p = bisa.Project(os.path.join(test_location, "x86_64", "ln_gcc_-O2"), auto_load_libs=False)
        ...
        if is_linux_x64():
            ...
            tempdir = tempfile.mkdtemp(prefix="bisa_test_reassembler_")
    ...
    def test_chmod_gcc_O1(self):
        ...
        p = bisa.Project(os.path.join(test_location, "x86_64", "chmod_gcc_-O1"), auto_load_libs=False)
        r = p.analyses.Reassembler(syntax="at&t")
        r.symbolize()
        r.remove_unnecessary_stuff()
        assembly = r.assembly(comments=True, symbolized=True)
    ...
    def test_ex_gpp(self):
        ...
        p = bisa.Project(os.path.join(test_location, "x86_64", "ex_g++"), auto_load_libs=False)
        r = p.analyses.Reassembler(syntax="at&t")
        r.symbolize()
        r.remove_unnecessary_stuff()
        assembly = r.assembly(comments=True, symbolized=True)
    ...
    def test_dir_gcc_O0(self):
        ...
        p = bisa.Project(os.path.join(test_location, "x86_64", "dir_gcc_-O0"), auto_load_libs=False)
        r = p.analyses.Reassembler(syntax="at&t")
        r.symbolize()
        r.remove_unnecessary_stuff()
        assembly = r.assembly(comments=True, symbolized=True)
    ...
    def test_helloworld(self):
        ...
        # https://github.com/bisa/bisa/issues/1630

        p = bisa.Project(os.path.join(test_location, "x86_64", "hello_world"), auto_load_libs=False)
        r = p.analyses.Reassembler(syntax="at&t")
        r.symbolize()
        r.remove_unnecessary_stuff()
    ...
    def test_helloworld_gcc9(self):
        ...
        # https://github.com/bisa/patcherex/issues/39

        p = bisa.Project(os.path.join(test_location, "x86_64", "hello_gcc9_reassembler"), auto_load_libs=False)
        r = p.analyses.Reassembler(syntax="at&t")
        r.symbolize()
        r.remove_unnecessary_stuff()
        assembly = r.assembly(comments=True, symbolized=True)
    ...
    def test_partial_pie_ls_x86(self):
        ...
        p = bisa.Project(os.path.join(test_location, "i386", "ls_gcc_7.5_reassembler"), auto_load_libs=False)
        r = p.analyses.Reassembler(syntax="at&t")
        r.symbolize()
        r.remove_unnecessary_stuff()
        assembly = r.assembly(comments=True, symbolized=True)
