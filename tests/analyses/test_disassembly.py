...
import bisa
from bisa.analyses import Disassembly
from bisa.analyses.disassembly import MemoryOperand, Instruction
from bisa.errors import BISATypeError
...
class TestDisassembly(TestCase):
    def test_capstone_unsupported(self):
        ...
        expected_message = (
            f"Cannot disassemble block with architecture {arch} for block type <class 'bisa.codenode.BlockNode'>"
        )
        try:
    ...
    def test_arm32_thumb_dissect_instructions(self):
        proj = bisa.load_shellcode(b"\x00\xf9\x01\x1a", "ARM", 0, thumb=True)
        # vst1.8  {d1, d2}, [r0], r1
