...
import bisa
from bisa.analyses import ReachingDefinitionsAnalysis, CFGFast, CompleteCallingConventionsAnalysis
from bisa.code_location import CodeLocation, ExternalCodeLocation
from bisa.analyses.reaching_definitions.rd_state import ReachingDefinitionsState
from bisa.analyses.reaching_definitions.subject import Subject
from bisa.analyses.reaching_definitions.dep_graph import DepGraph
from bisa.analyses.reaching_definitions.function_handler_library import LibcHandlers
from bisa.block import Block
from bisa.engines.light import SpOffset
from bisa.knowledge_plugins.key_definitions import DerefSize
from bisa.knowledge_plugins.key_definitions.live_definitions import LiveDefinitions
from bisa.knowledge_plugins.key_definitions.atoms import AtomKind, GuardUse, Tmp, Register, MemoryLocation
from bisa.knowledge_plugins.key_definitions.constants import ObservationPointType, OP_BEFORE, OP_AFTER
from bisa.knowledge_plugins.key_definitions import Definition
...
class InsnAndNodeObserveTestingUtils:
    ...
    def setup(observation_points):
        ...
        project = bisa.Project(binary_path, load_options={"auto_load_libs": False})
...
class TestReachingDefinitions(TestCase):
    def _run_reaching_definition_analysis_test(self, project, function, result_path, _extract_result):
        tmp_kb = bisa.KnowledgeBase(project)
    ...
    def test_reaching_definition_analysis_definitions(self):
        ...
        for binary, result_path in binaries_and_results:
            project = bisa.Project(binary, load_options={"auto_load_libs": False})
            cfg = project.analyses[CFGFast].prep()()
    ...
    def test_insn_observe_after_a_function_call(self):
        ...
        project = bisa.Project(bin_path, auto_load_libs=False)
        cfg = project.analyses[CFGFast].prep()()
    ...
    def test_get_the_sp_from_a_reaching_definition(self):
        ...
        project = bisa.Project(binary, auto_load_libs=False)
        cfg = project.analyses[CFGFast].prep()()
    ...
    def test_dep_graph(self):
        project = bisa.Project(_binary_path("true"), auto_load_libs=False)
        cfg = project.analyses[CFGFast].prep()()
        main_func = cfg.functions["main"]
    ...
    def test_constants_not_stored_to_live_memory_defs(self):
        ...
        project = bisa.Project(_binary_path("two_cond_func_call_with_const_arg", "armel"), auto_load_libs=False)
        ...
        for info in rda.callsites_to("f"):
            ...
            # Verify not stored
            with self.assertRaises(bisa.errors.SimMemoryMissingError):
                atom = preds[0].atom
