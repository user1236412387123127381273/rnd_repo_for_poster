...
from bisa.analyses.decompiler.peephole_optimizations import PeepholeOptimizationStmtBase
...
class BadOptimizationPass(bisa.analyses.decompiler.optimization_passes.optimization_pass.OptimizationPass):
    def __init__(self, *args, **kwargs):
        raise BadOptimizerException("bad")

    STAGE = bisa.analyses.decompiler.optimization_passes.OptimizationPassStage.AFTER_AIL_GRAPH_CREATION
...
class TestDecompilerErrors(unittest.TestCase):
    def test_errors(self):
        ...
        try:
            _result = p.analyses[bisa.analyses.Decompiler].prep(fail_fast=True)(
        ...
        try:
            decomp = p.analyses[bisa.analyses.Decompiler].prep(fail_fast=False)(
                func=main, optimization_passes=[BadOptimizationPass], peephole_optimizations=[BadPeepholeOptimization]
            )
