...
class TestStructMemberAccess(unittest.TestCase):
    def test_struct_member_write(self):
        ...
        proj = bisa.Project(bin_path, auto_load_libs=False, load_debug_info=True)
        ...
        bisa.types.register_types(bisa.types.parse_type("struct Inner {long long a; int b;}"))
        bisa.types.register_types(bisa.types.parse_type("struct Outer {char* a; struct Inner b;}"))

        foo_func.calling_convention = default_cc(
            proj.arch.name, platform=proj.simos.name if proj.simos is not None else None
        )(proj.arch)
        foo_func.prototype = bisa.types.parse_type("void (struct Outer *a)").with_arch(proj.arch)
