...
class TestSymbolic(unittest.TestCase):
    ...
    def test_concretization_strategies(self):
        ...
        s = bisa.SimState(arch="AMD64", dict_memory_backer=initial_memory)
        ...
        x = claripy.BVS("x", s.arch.bits)
        s.add_constraints(x >= 1)
        ss.options.add(bisa.options.CONSERVATIVE_READ_STRATEGY)
    ...
    #   s = bisa.SimState(arch="AMD64", mode="symbolic")
    ...
    def test_unsat_core(self):
        s = bisa.SimState(arch="AMD64", mode="symbolic", add_options={bisa.options.CONSTRAINT_TRACKING_IN_SOLVER})
        x = claripy.BVS("x", 32)
