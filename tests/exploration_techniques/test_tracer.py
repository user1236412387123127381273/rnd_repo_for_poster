...
def tracer_cgc(
    ...
    p = bisa.Project(filename)
    p.simos.syscall_library.update(bisa.SIM_LIBRARIES["cgcabi_tracer"][0])

    trace, magic, crash_mode, crash_addr = do_trace(p, test_name, stdin)
    s = p.factory.entry_state(
        mode="tracing",
        stdin=bisa.SimFileStream,
        flag_page=magic,
    ...
    t = bisa.exploration_techniques.Tracer(
        trace,
    ...
    if add_options is not None and bisa.options.UNICORN_HANDLE_CGC_RECEIVE_SYSCALL in add_options:
        fd_data = {0: (stdin, b"\x01" * len(stdin))} if symbolic_stdin else {0: (stdin, b"\x00" * len(stdin))}

        t.set_fd_data(fd_data)

    simgr.use_technique(t)
    simgr.use_technique(bisa.exploration_techniques.Oppologist())
...
def tracer_linux(filename, test_name, stdin, add_options=None, remove_options=None):
    ...
    simgr = p.factory.simulation_manager(s, hierarchy=None, save_unconstrained=crash_mode)
    t = bisa.exploration_techniques.Tracer(trace, crash_addr=crash_addr)
    simgr.use_technique(t)
    simgr.use_technique(bisa.exploration_techniques.Oppologist())
...
class TestTracer(unittest.TestCase):
    ...
    def test_cgc_receive_unicorn_native_interface(self):
        ...
        add_options = {
            bisa.options.UNICORN_HANDLE_CGC_RECEIVE_SYSCALL,
            bisa.options.UNICORN_HANDLE_SYMBOLIC_ADDRESSES,
            bisa.options.UNICORN_HANDLE_SYMBOLIC_CONDITIONS,
            bisa.options.UNICORN_HANDLE_SYMBOLIC_SYSCALLS,
        }
    ...
    def test_cgc_random_syscall_handling_native_interface(self):
        ...
        add_options = {
            bisa.options.UNICORN_HANDLE_CGC_RECEIVE_SYSCALL,
            bisa.options.UNICORN_HANDLE_CGC_RANDOM_SYSCALL,
            bisa.options.UNICORN_HANDLE_SYMBOLIC_ADDRESSES,
            bisa.options.UNICORN_HANDLE_SYMBOLIC_CONDITIONS,
            bisa.options.UNICORN_HANDLE_SYMBOLIC_SYSCALLS,
        }
    ...
    def test_cgc_se1_palindrome_raw(self):
        ...
        # make sure bisa modeled the correct output
        stdout_dump = simgr.traced[0].posix.dumps(1)
    ...
    def test_fauxware(self):
        ...
        simgr, _ = tracer_linux(b, "tracer_fauxware", b"A" * 18, remove_options={bisa.options.CPUID_SYMBOLIC})
        simgr.run()
    ...
    def test_floating_point_memory_reads(self):
        ...
        trace_cgc_with_pov_file(
            ...
            pov_file,
            output,
            read_strategies=[bisa.concretization_strategies.SimConcretizationStrategyAny(exact=True)],
            write_strategies=[bisa.concretization_strategies.SimConcretizationStrategyAny(exact=True)],
        )
    ...
    def test_concretize_unsupported_vex_irops(self):
        ...
        add_options = {
            bisa.options.UNSUPPORTED_FORCE_CONCRETIZE,
            bisa.options.UNICORN_HANDLE_CGC_RECEIVE_SYSCALL,
            bisa.options.UNICORN_HANDLE_SYMBOLIC_ADDRESSES,
            bisa.options.UNICORN_HANDLE_SYMBOLIC_CONDITIONS,
        }
...
if __name__ == "__main__":
    logging.getLogger("bisa.simos").setLevel("DEBUG")
    logging.getLogger("bisa.state_plugins.preconstrainer").setLevel("DEBUG")
    logging.getLogger("bisa.exploration_techniques.tracer").setLevel("DEBUG")
    logging.getLogger("bisa.exploration_techniques.crash_monitor").setLevel("DEBUG")
