...
import bisa
from bisa.sim_type import SimTypePointer, SimTypeChar
...
class TestDirector(unittest.TestCase):
    def test_execute_address_brancher(self):
        p = bisa.Project(os.path.join(test_location, "x86_64", "brancher"), load_options={"auto_load_libs": False})

        pg = p.factory.simulation_manager()

        # initialize the exploration technique
        dm = bisa.exploration_techniques.Director(num_fallback_states=1)
        goal = bisa.exploration_techniques.ExecuteAddressGoal(0x400594)
        dm.add_goal(goal)
        pg.use_technique(dm)
    ...
    def test_call_function_brancher(self):
        ...
        # initialize the exploration technique
        dm = bisa.exploration_techniques.Director(
        ...
        goal = bisa.exploration_techniques.CallFunctionGoal(puts_func, [(SimTypePointer(SimTypeChar()), ">=20")])
        dm.add_goal(goal)
        pg.use_technique(dm)
...
if __name__ == "__main__":
    logging.getLogger("bisa.exploration_techniques.director").setLevel(logging.DEBUG)
    unittest.main()
