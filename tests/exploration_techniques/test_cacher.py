...
l = logging.getLogger("bisa_tests.managers")
...
class TestCacher(unittest.TestCase):
    ...
    def test_cacher(self):
        p = bisa.Project(os.path.join(test_location, "x86_64", "fauxware"), load_options={"auto_load_libs": False})
        ...
        pg.use_technique(bisa.exploration_techniques.Cacher(when=0x4006EE, container=container))
        pg.run()

        pg2 = p.factory.simulation_manager()
        pg2.use_technique(bisa.exploration_techniques.Cacher(container=container))
