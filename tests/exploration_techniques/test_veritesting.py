...
l = logging.getLogger("bisa_tests.veritesting")
...
class TestVeritesting(unittest.TestCase):
    def _run_veritesting_a(self, arch):
        ...
        # logging.getLogger('bisa.analyses.sse').setLevel(logging.DEBUG)
    ...
    def _run_veritesting_b(self, arch):
        ...
        ex.use_technique(bisa.exploration_techniques.Veritesting(enable_function_inlining=True))
    ...
    def _run_veritesting_skm(self, arch):
        proj = bisa.Project(os.path.join(test_location, arch, "veritesting_skm"))

        # start the analysis after the call to lexer_read_line
        state = proj.factory.blank_state(addr=0x4024AE, remove_options={bisa.sim_options.UNICORN})
