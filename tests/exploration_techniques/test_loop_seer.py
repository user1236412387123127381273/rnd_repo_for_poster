...
class TestLoopSeer(unittest.TestCase):
    def test_various_loops(self):
        p = bisa.Project(os.path.join(test_location, "x86_64", "various_loops"), auto_load_libs=False)

        cfg = p.analyses.CFGFast(normalize=True)

        state = p.factory.entry_state()
        state.register_plugin("loop_data", bisa.state_plugins.SimStateLoopData())
        ...
        simgr.use_technique(bisa.exploration_techniques.LoopSeer(cfg=cfg, functions=None, bound=None))
    ...
    def test_loops_with_invalid_parameter(self):
        p = bisa.Project(os.path.join(test_location, "x86_64", "test_loops"), auto_load_libs=False)

        state = p.factory.entry_state()
        state.register_plugin("loop_data", bisa.state_plugins.SimStateLoopData())
        simgr = p.factory.simulation_manager(state)

        simgr.use_technique(bisa.exploration_techniques.LoopSeer(functions=["main", 0x1234], bound=None))
    ...
    def test_arrays(self):
        p = bisa.Project(os.path.join(test_location, "x86_64", "test_arrays"), auto_load_libs=False)
        ...
        simgr.use_technique(bisa.exploration_techniques.LoopSeer(cfg=cfg, functions="main", bound=None))
    ...
    def test_loop_limiter(self):
        ...
        simgr.use_technique(bisa.exploration_techniques.LoopSeer(cfg=cfg, functions="main", bound=5))
    ...
    def test_loop_limiter_constant_loop(self):
        p = bisa.Project(os.path.join(test_location, "x86_64", "constant_loopseer"), auto_load_libs=False)
        ...
        simgr.use_technique(
            bisa.exploration_techniques.LoopSeer(cfg=cfg, functions="main", bound=5, limit_concrete_loops=False)
        )
