...
class TestPcodeEngine(TestCase):
    def test_shellcode(self):
        ...
        bisa.calling_conventions.register_default_cc(arch.name, bisa.calling_conventions.SimCCSystemVAMD64)
        p = bisa.load_shellcode(code, arch=arch, load_address=base_address, engine=bisa.engines.UberEnginePcode)
    ...
    def test_fauxware(self):
        ...
        """
        p = bisa.Project(
            os.path.join(test_location, "x86_64", "fauxware"), auto_load_libs=False, engine=bisa.engines.UberEnginePcode
        )
