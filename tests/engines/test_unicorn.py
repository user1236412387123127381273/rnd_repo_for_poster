...
import bisa
from bisa import options as so
...
class TestUnicorn(unittest.TestCase):
    def test_stops(self):
        p = bisa.Project(os.path.join(test_location, "i386", "uc_stop"), auto_load_libs=False)
        ...
        s_normal_bisa = p.factory.entry_state(args=["a"])
        pg_normal_bisa = p.factory.simulation_manager(s_normal_bisa).run()
        p_normal_bisa = pg_normal_bisa.one_deadended
        assert p_normal_bisa.history.bbl_addrs.hardcopy == p_normal.history.bbl_addrs.hardcopy

        # test STOP_STOPPOINT on an address that is not a basic block start
        s_stoppoints = p.factory.call_state(
            p.loader.find_symbol("main").rebased_addr, 1, bisa.PointerWrapper([]), add_options=so.unicorn
        )
        ...
        s_symbolic_read_tracking_disabled_bisa = p.factory.entry_state(args=["a", "a"])
        pg_symbolic_read_tracking_disabled_bisa = p.factory.simulation_manager(
            s_symbolic_read_tracking_disabled_bisa
        ).run()
        p_symbolic_read_tracking_disabled_bisa = pg_symbolic_read_tracking_disabled_bisa.one_deadended
        assert (
            p_symbolic_read_tracking_disabled_bisa.history.bbl_addrs.hardcopy
        ...
        s_segfault_bisa = p.factory.entry_state(
            args=["a", "a", "a", "a", "a", "a", "a"],
            add_options={so.STRICT_PAGE_ACCESS, so.ENABLE_NX},
        )
        pg_segfault_bisa = p.factory.simulation_manager(s_segfault_bisa).run()
        p_segfault_bisa = pg_segfault_bisa.errored[0].state
        assert p_segfault_bisa.history.bbl_addrs.hardcopy == p_segfault.history.bbl_addrs.hardcopy
        assert pg_segfault_bisa.errored[0].error.addr == pg_segfault.errored[0].error.addr
        ...
        s_symbolic_exit_bisa = p.factory.entry_state(args=["a"] * 10)
        pg_symbolic_exit_bisa = p.factory.simulation_manager(s_symbolic_exit_bisa).run()
        p_symbolic_exit_bisa = pg_symbolic_exit_bisa.one_deadended
        assert p_symbolic_exit_bisa.history.bbl_addrs.hardcopy == p_symbolic_exit.history.bbl_addrs.hardcopy

    @staticmethod
    def _run_longinit(arch):
        p = bisa.Project(os.path.join(test_location, arch, "longinit"), auto_load_libs=False)
    ...
    def test_fauxware_arm(self):
        p = bisa.Project(os.path.join(test_location, "armel", "fauxware"), auto_load_libs=False)
    ...
    def test_fauxware(self):
        p = bisa.Project(os.path.join(test_location, "i386", "fauxware"), auto_load_libs=False)
        s_unicorn = p.factory.entry_state(add_options=so.unicorn)  # unicorn
        pg = p.factory.simulation_manager(s_unicorn)
        pg.explore()
    ...
    def _run_similarity(binpath, depth, prehook=None):
        b = bisa.Project(os.path.join(test_location, binpath), auto_load_libs=False)
    ...
    def test_fp(self):
        with open(os.path.join(bin_location, "tests_src", "manyfloatsum.c"), encoding="utf-8") as fp:
            type_cache = bisa.sim_type.parse_defns(fp.read())
        p = bisa.Project(os.path.join(test_location, "i386", "manyfloatsum"), auto_load_libs=False)
    ...
    def test_concrete_transmits(self):
        p = bisa.Project(os.path.join(test_location, "cgc", "PIZZA_00001"), auto_load_libs=False)
    ...
    def test_inspect(self):
        ...
        def main_state(argc, add_options=None):
            ...
            return p.factory.call_state(main_addr, argc, bisa.PointerWrapper([]), add_options=add_options)
    ...
    def test_symbolic_flags_preserved_on_stop(self):
        ...
        p = bisa.Project(os.path.join(test_location, "x86_64", "test_symbolic_flags_in_unicorn"))
        init_state = p.factory.full_init_state(add_options=bisa.options.unicorn)
...
if __name__ == "__main__":
    ...
    logging.getLogger("bisa.state_plugins.unicorn_engine").setLevel("DEBUG")
    logging.getLogger("bisa.engines.unicorn_engine").setLevel("INFO")
    logging.getLogger("bisa.factory").setLevel("DEBUG")
    logging.getLogger("bisa.project").setLevel("DEBUG")
