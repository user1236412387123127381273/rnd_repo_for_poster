...
class TestOps(unittest.TestCase):
    def test_irop_perm(self):
        p = bisa.load_shellcode("vpshufb xmm0,xmm1,xmm2", "amd64")
    ...
    def test_irop_mulhi(self):
        p = bisa.load_shellcode("vpmulhw xmm0,xmm1,xmm2", "amd64")
    ...
    def test_irop_catevenlanes(self):
        p = bisa.load_shellcode("pmulhrsw xmm0, xmm1", "amd64")
    ...
    def test_saturating_packing(self):
        ...
        p = bisa.load_shellcode("vpackuswb xmm1, xmm0, xmm0", arch="amd64")
        ...
        p = bisa.load_shellcode("vpackusdw xmm1, xmm0, xmm0", arch="amd64")
        s = p.factory.blank_state()
        ...
        p = bisa.load_shellcode("vpacksswb xmm1, xmm0, xmm0", arch="amd64")
        s = p.factory.blank_state()
        s.regs.xmm0 = 0x0000_0001_7FFE_7FFF_8000_8001_FFFE_FFFF
        s = s.step(num_inst=1).successors[0]
