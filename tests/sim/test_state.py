...
class TestState(unittest.TestCase):
    ...
    def test_state_merge(self):
        ...
        # logging.getLogger('bisa.state_plugins.symbolic_memory').setLevel(logging.DEBUG)
        m, merge_conditions, merging_occurred = a.merge(b, c)
        # logging.getLogger('bisa.state_plugins.symbolic_memory').setLevel(logging.WARNING)
        ...
        self.assertRaises(bisa.errors.SimMergeError, lambda: a.copy().merge(b.copy()))
    ...
    def test_state_merge_optimal(self):
        ...
        binary_path = os.path.join(test_location, "x86_64", "state_merge_0")
        p = bisa.Project(binary_path, auto_load_libs=False)
        state = p.factory.blank_state(add_options={bisa.sim_options.EFFICIENT_STATE_MERGING})
    ...
    def test_successors_catch_arbitrary_interrupts(self):
        ...
        proj = bisa.load_shellcode(block_bytes, "amd64")
        proj.loader._tls = cle.backends.tls.ELFThreadManager(proj.loader, proj.arch)
        proj.simos = bisa.simos.SimLinux(proj)
        ...
        assert (
            len(simgr.errored) == 0
        ), "The state should not go to the errored stash. Is BISASyscallError handled in SimSuccessors?"
    ...
    def test_bypass_errored_irstmt(self):
        ...
        proj = bisa.load_shellcode(block_bytes, "x86")
        state = proj.factory.blank_state(
            addr=0,
            mode="fastpath",
            cle_memory_backer=proj.loader.memory,
            add_options={bisa.sim_options.FAST_REGISTERS},
            remove_options={bisa.sim_options.BYPASS_ERRORED_IRSTMT},
        )
        ...
        state.options.add(bisa.sim_options.BYPASS_ERRORED_IRSTMT)
        simgr = proj.factory.simgr(state)
        simgr.step()
