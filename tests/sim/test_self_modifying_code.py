...
class TestSelfModifyingCOde(TestCase):
    def test_self_modifying_code(self):
        p = bisa.Project(os.path.join(test_location, "cgc", "stuff"), auto_load_libs=False, selfmodifying_code=True)
        ...
        # the two histories are not the same because bisa does not add relifted block addresses (caused by raising
        ...
        while i < len(pg_bbl_addrs) and j < len(u_bbl_addrs):
            if pg_bbl_addrs[i] == u_bbl_addrs[j]:
                ...
                # this is the missing relifted block address in bisa's history. skip it
                j += 1
            else:
    ...
    def test_self_modifying_code_overwrite_invalid_instruction(self):
        ...
        proj = bisa.load_shellcode(code, "amd64", selfmodifying_code=True)
        state = proj.factory.blank_state(addr=0, add_options={bisa.options.SYMBOL_FILL_UNCONSTRAINED_REGISTERS})
