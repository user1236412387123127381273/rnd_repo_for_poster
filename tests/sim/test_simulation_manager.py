...
class TestSimulationManager(unittest.TestCase):
    def _run_fauxware(self, arch, threads):
        p = bisa.Project(os.path.join(test_location, arch, "fauxware"), load_options={"auto_load_libs": False})
    ...
    def test_explore_with_cfg(self):
        ...
        pg = p.factory.simulation_manager()
        pg.use_technique(bisa.exploration_techniques.Explorer(find=0x4006ED, cfg=cfg, num_find=3))
        pg.run()
