...
class TestAccuracy(unittest.TestCase):
    def _emulate(self, arch, binary, use_sim_procs, steps, hit_addrs, finished):
        ...
        state = p.factory.full_init_state(
            args=["./test_arrays"],
            add_options={
                bisa.options.STRICT_PAGE_ACCESS,
                bisa.options.ENABLE_NX,
                bisa.options.ZERO_FILL_UNCONSTRAINED_MEMORY,
                bisa.options.USE_SYSTEM_TIMES,
            },
        )
    ...
    def test_locale(self):
        # auto_load_libs can't be disabled as the test takes longer time to execute
        p = bisa.Project(os.path.join(test_location, "i386", "isalnum"), use_sim_procedures=False, auto_load_libs=True)
        state = p.factory.full_init_state(args=["./isalnum"], add_options={bisa.options.STRICT_PAGE_ACCESS})
