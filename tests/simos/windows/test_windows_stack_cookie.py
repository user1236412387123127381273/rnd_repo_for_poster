...
def compare_none(state, test_value):
    ...
    correct_value = bisa.simos.windows.VS_SECURITY_COOKIES[state.arch.name].default
    assert test_value == correct_value


def compare_random(state, test_value):
    test_value = test_value.concrete
    incorrect_value = bisa.simos.windows.VS_SECURITY_COOKIES[state.arch.name].default
...
class TestWindowsStackCookie(unittest.TestCase):
    def test_security_cookie_init(self):
        project = bisa.Project(os.path.join(test_location, "i386", "test_arrays.exe"), auto_load_libs=False)
        check_value(project, bisa.simos.windows.SecurityCookieInit.NONE, compare_none)
        check_value(project, bisa.simos.windows.SecurityCookieInit.RANDOM, compare_random)
        check_value(project, bisa.simos.windows.SecurityCookieInit.STATIC, compare_static)
        check_value(project, bisa.simos.windows.SecurityCookieInit.SYMBOLIC, compare_symbolic)
