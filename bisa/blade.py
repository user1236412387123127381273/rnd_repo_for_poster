...
from .errors import BISABladeError, SimTranslationError
...
class Blade:
    ...
    It is meant to be used in bisa for small or on-the-fly analyses.
    """
    ...
    def __init__(
        ...
        :param graph:                   A graph representing the control flow graph. Note that it does not take
                                        bisa.analyses.CFGEmulated or bisa.analyses.CFGFast.
        ...
        :param bisa.Project project:    The project instance.
        :param bisa.analyses.CFGBase cfg: the CFG instance. It will be made mandatory later.
        ...
        if self._cfg is None:
            ...
            raise BISABladeError('"cfg" must be specified.')

        if not self._in_graph(self._dst_run):
            raise BISABladeError(f"The specified SimRun {self._dst_run} doesn't exist in graph.")
        ...
        if direction == "backward":
            ...
            raise BISABladeError("Forward slicing is not implemented yet")
        else:
            raise BISABladeError(f"Unknown slicing direction {direction}")
    ...
    def _get_irsb(self, v):
        ...
        if type(v) is int:
            ...
            if self.project:
                ...
                return irsb
            raise BISABladeError("Project must be specified if you give me all addresses for SimRuns")

        raise BISABladeError(f"Unsupported SimRun argument type {type(v)}")
    ...
    def _backward_slice(self):
        ...
        if self._dst_stmt_idx != -1:
            ...
            if type(dst_stmt) is pyvex.IRStmt.Put:
                ...
                raise BISABladeError("Incorrect type of the specified target statement. We only support Put and WrTmp.")
            ...
            if type(next_expr) is pyvex.IRExpr.RdTmp:
                ...
                raise BISABladeError(f"Unsupported type for irsb.next: {type(next_expr)}")
