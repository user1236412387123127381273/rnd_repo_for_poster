...
class Server:
    ...
    :ivar project:          An instance of bisa.Project.
    ...
    def __init__(
        ...
        self.spill_yard = spill_yard if spill_yard else tempfile.mkdtemp(suffix="bisa_spill_yard")
        if not spill_yard:
            _l.info("Temporary spill yard: %s", self.spill_yard)
        self.db_str = db if db else "sqlite:///" + os.path.join(tempfile.mkdtemp(suffix="bisa_server_db"), "db.sqlite3")
        if not db:
