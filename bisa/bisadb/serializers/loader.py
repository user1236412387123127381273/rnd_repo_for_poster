...
from bisa.errors import BISACorruptDBError, BISADBError
from bisa.bisadb.models import DbObject
...
class LoaderSerializer:
    """
    Serialize/unserialize a CLE Loader object into/from an bisa DB.

    Corner cases:
    - For certain backends (e.g., CART), we do not store the data of the main object. bisa will unpack the CART file
    ...
    def dump(session, loader):
        ...
        for obj in loader.all_objects:
            ...
            try:
                ...
                raise BISADBError(f"Failed to load content for file {obj.binary}.") from ex
    ...
    def load(session):
        ...
        if main_object is None:
            raise BISACorruptDBError("Corrupt database: No main object.")
