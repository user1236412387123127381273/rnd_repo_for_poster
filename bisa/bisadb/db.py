...
from bisa.errors import BISACorruptDBError, BISAIncompatibleDBError, BISADBError
...
class BISADB:
    """
    BISADB provides a storage solution for an bisa project, its knowledge bases, and some other types of data. It is
    ...
    def open_db(db_str="sqlite:///:memory:"):
        try:
            ...
            raise BISACorruptDBError("The target file may not be an bisa database or it is corrupted.") from ex
        except Exception as ex:
            raise BISADBError from ex
    ...
    def db_compatible(self, version):
        """
        Checks if the given database version is compatible with the current BISADB class.
    ...
    def load(
        ...
        with self.open_db(db_str) as Session, self.session_scope(Session) as session:
            ...
            if not self.db_compatible(dbinfo.get("version", None)):
                raise BISAIncompatibleDBError(
                    "Version {} is incompatible with the current version of bisa.".format(dbinfo.get("version", None))
                )
            ...
            if (len(kb_names) != 1 or kb_names[0] != "global") and other_kbs is None:
                raise ValueError(
                    'You must provide a dict via "other_kbs" to collect bisa KnowledgeBases '
