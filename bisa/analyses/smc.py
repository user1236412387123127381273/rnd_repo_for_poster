...
from bisa.utils.tagged_interval_map import TaggedIntervalMap
...
class TraceClassifier:
    ...
    def instrument(self, state) -> None:
        ...
        """
        state.inspect.b("mem_write", when=bisa.BP_BEFORE, action=self.act_mem_write)
        state.inspect.b("instruction", when=bisa.BP_BEFORE, action=self.act_instruction)
...
class SelfModifyingCodeAnalysis(Analysis):
    ...
    def __init__(self, subject: None | int | str | Function, max_bytes: int = 0, state: SimState | None = None):
        ...
        init_state.options -= bisa.sim_options.simplification
