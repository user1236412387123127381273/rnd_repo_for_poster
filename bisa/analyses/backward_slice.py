...
from bisa.errors import BISABackwardSlicingError
...
class BackwardSlice(Analysis):
        ...
        if targets is not None:
            for t in targets:
                if isinstance(t, CodeLocation):
                    ...
                    raise BISABackwardSlicingError(f"Unsupported type of target {t}")
    ...
    def is_taint_related_to_ip(self, simrun_addr, stmt_idx, taint_type, simrun_whitelist=None):
        ...
        if taint is None:
            raise BISABackwardSlicingError("The specific taint is not found")
    ...
    def _construct_default(self, targets):
        ...
        for cfg_node, stmt_idx in targets:
            if cfg_node not in self._cfg.graph:
                raise BISABackwardSlicingError(f"Target CFGNode {cfg_node} is not in the CFG.")
    ...
    def _pick_statement(self, block_address, stmt_idx):
        ...
        if not isinstance(block_address, int):
            raise BISABackwardSlicingError(f"Invalid block address {block_address}.")
        if not isinstance(stmt_idx, int):
            raise BISABackwardSlicingError(f"Invalid statement ID {stmt_idx}.")
    ...
    def _normalize_stmt_idx(self, block_addr, stmt_idx):
        ...
        raise BISABackwardSlicingError(f'Unsupported statement ID "{stmt_idx}"')
