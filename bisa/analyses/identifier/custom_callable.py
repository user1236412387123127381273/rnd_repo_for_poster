...
from bisa.errors import BISACallableError, BISACallableMultistateError
    ...
    def perform_call(self, *args, prototype=None):
        ...
        for _ in range(self._max_steps):
            ...
            if caller.active[0].history.block_count > 100000:
                ...
                raise BISACallableError("Super long path")
            caller.step(step_func=step_func if self._concrete_only else None)
        if len(caller.active) > 0:
            raise BISACallableError("didn't make it to the end of the function")
