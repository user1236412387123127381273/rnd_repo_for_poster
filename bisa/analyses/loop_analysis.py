...
from bisa.block import SootBlockNode
from bisa.errors import BISALoopAnalysisError
...
class SootBlockProcessor:
    ...
    def process(self):
        if not isinstance(self.block, SootBlockNode):
            raise BISALoopAnalysisError(f"Got an unexpected type of block {type(self.block)}.")
