...
from bisa.errors import BISADDGError, BISAAnalysisError, SimValueError
from bisa.state_plugins import SimActionData
from bisa.storage import DefaultMemory
...
class DataDependencyGraphAnalysis(Analysis):
    ...
    def __init__(
        ...
        if self._block_addrs and self._end_at:
            raise BISADDGError("Can not specify BOTH start/end addresses and a block list!")
    ...
    def _get_generic_node(self, action: SimActionData) -> BaseDepNode:
        def node_attributes(act: SimActionData) -> tuple:
            ...
            if ty not in [SimActionData.REG, SimActionData.TMP, SimActionData.MEM]:
                raise BISAAnalysisError(f"Unsupported type: <{ty}>!")
            if ac not in [SimActionData.WRITE, SimActionData.READ]:
                raise BISAAnalysisError(f"Unsupported action type: <{ac}>!")
    ...
    def _parse_mem_statement(self, read_nodes: dict[int, list[BaseDepNode]] | None = None) -> SimActLocation:
        ...
        if act.action == SimActionData.WRITE:
            ...
            if src_nodes:
                ...
                raise BISAAnalysisError(f"Unexpected MemWrite pattern encountered! <{act}>")
    ...
    def _parse_statement(self, read_nodes: dict[int, list[BaseDepNode]] | None = None) -> SimActLocation:
        ...
        if sim_act.action not in [SimActionData.WRITE, SimActionData.READ]:
            raise BISAAnalysisError(f"Statement with unsupported action encountered: <{sim_act.action}>")
        if sim_act.type not in [SimActionData.TMP, SimActionData.MEM, SimActionData.REG]:
            raise BISAAnalysisError(f"Statement with unsupported type encountered: <{sim_act.type}>")
        if (
