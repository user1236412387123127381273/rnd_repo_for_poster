...
from bisa.errors import BISADDGError
from bisa.sim_variable import SimRegisterVariable, SimMemoryVariable
...
class VSA_DDG(Analysis):
    ...
    def __init__(
        ...
        if vfg is None and start_addr is None:
            raise BISADDGError("Argument vfg and start_addr cannot both be unspecified.")

        if vfg is not None:
            self._vfg = vfg
        else:
            self._vfg = self.project.analyses[VFG].prep()(
                function_start=start_addr,
        ...
        for a in action_list:
            ...
            if a.type == "mem":
                if a.actual_addrs is None:
                    ...
                    addr_list = {
                        aw.to_valueset(state) for aw in state.memory._concretize_address_descriptor(a.addr.ast)
                    }
                else:
