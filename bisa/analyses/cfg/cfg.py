...
class CFG(CFGFast):  # pylint: disable=abstract-method
    ...
    For multiple historical reasons, bisa's CFG is accurate but slow, which does not meet what most people expect. We
    ...
    However, so many new users of bisa would load up a binary and generate a CFG immediately after running
    "pip install bisa", and draw the conclusion that "bisa's CFG is so slow - bisa must be unusable!" Therefore, we made
    ...
    - You will see both a warning message printed out to stderr and an exception raised by bisa if you are passing CFG
      any parameter that only CFGEmulated supports. This exception is not a sub class of BISAError, so you wouldn't
