...
from bisa.analyses import ForwardAnalysis
from bisa.utils.graph import GraphUtils
from bisa.analyses import AnalysesHub
from bisa import BP, BP_BEFORE, BP_AFTER, SIM_PROCEDURES, procedures
from bisa import options as o
from bisa.codenode import BlockNode
from bisa.engines.procedure import ProcedureEngine
from bisa.exploration_techniques.loop_seer import LoopSeer
from bisa.exploration_techniques.slicecutor import Slicecutor
from bisa.exploration_techniques.explorer import Explorer
from bisa.exploration_techniques.lengthlimiter import LengthLimiter
from bisa.errors import (
    BISACFGError,
    BISAError,
    BISASkipJobNotice,
    SimError,
...
from bisa.state_plugins.callstack import CallStack
from bisa.state_plugins.sim_action import SimActionData
from bisa.knowledge_plugins.cfg import CFGENode, IndirectJump
from bisa.utils.constants import DEFAULT_STATEMENT
from bisa.analyses.cdg import CDG
from bisa.analyses.ddg import DDG
from bisa.analyses.backward_slice import BackwardSlice
from bisa.analyses.loopfinder import LoopFinder, Loop
from .cfg_base import CFGBase
from .cfg_job_base import BlockID, CFGJobBase

if TYPE_CHECKING:
    from bisa.knowledge_plugins.cfg import CFGNode
...
class CFGEmulated(ForwardAnalysis, CFGBase):  # pylint: disable=abstract-method
    ...
    def __init__(
        ...
        :param iterable starts:                     A collection of starting points to begin analysis. It can contain
                                                    ...
                                                    lead to an BISACFGError being raised.
        ...
        if start is not None:
            ...
            if isinstance(starts, (list, set)):
                ...
                raise BISACFGError("Unsupported type of the `starts` argument.")
    ...
    def force_unroll_loops(self, max_loop_unrolling_times):
        ...
        if start_node is None:
            raise BISACFGError("Cannot find start node when trying to unroll loops. The CFG might be empty.")
    ...
    def __setstate__(self, s):
        self.project: bisa.Project = s["project"]
    ...
    def deadends(self):
        ...
        if self.graph is None:
            raise BISACFGError("CFG hasn't been generated yet.")
    ...
    def _sanitize_parameters(self):
        ...
        An BISACFGError is raised if any parameter fails the sanity check.
        ...
        if isinstance(self._additional_edges, (list, set, tuple)):
            ...
            raise BISACFGError("Additional edges can only be a list, set, tuple, or a dict.")

        # Check _advanced_backward_slicing
        if self._advanced_backward_slicing and self._enable_symbolic_back_traversal:
            raise BISACFGError("Advanced backward slicing and symbolic back traversal cannot both be enabled.")

        if self._advanced_backward_slicing and not self._keep_state:
            raise BISACFGError("Keep state must be enabled if advanced backward slicing is enabled.")

        # Sanitize avoid_runs
        self._avoid_runs = [] if self._avoid_runs is None else self._avoid_runs
        if not isinstance(self._avoid_runs, (list, set)):
            raise BISACFGError('"avoid_runs" must either be None, or a list or a set.')
        ...
        if self._starts is None:
            ...
            for item in self._starts:
                if isinstance(item, tuple):
                    if len(item) != 2:
                        raise BISACFGError(f'Unsupported item in "starts": {item!s}')
                    ...
                    raise BISACFGError(f'Unsupported item type in "starts": {type(item)}')

            self._starts = new_starts

        if not self._starts:
            raise BISACFGError("At least one start must be provided")
    ...
    def _pre_analysis(self):
        ...
        for item in self._starts:
            ...
            if isinstance(item, tuple):
                ...
                raise BISACFGError(f"Unsupported CFG start type: {type(item)!s}.")
    ...
    def _pre_job_handling(self, job):  # pylint:disable=arguments-differ
        ...
        before tracing it.
        An BISAForwardAnalysisSkipJob exception is raised in order to skip analyzing the job.
    ...
    def _get_symbolic_function_initial_state(self, function_addr, fastpath_mode_state=None):
        ...
        if fastpath_mode_state is not None:
            ...
            if function_addr in self._function_input_states:
                ...
                raise BISACFGError("The impossible happened. Please report to Fish.")
    ...
    def _get_simsuccessors(self, addr, job, current_function_addr=None):
        ...
        try:
        ...
        except BISAExitError:
            exception_info = sys.exc_info()
            l.debug("Caught a BISAExitError during CFG recovery. Don't panic, this is usually expected.", exc_info=True)
            # Generate a PathTerminator to terminate the current path
            inst = SIM_PROCEDURES["stubs"]["PathTerminator"]()
            sim_successors = ProcedureEngine(self.project).process(state, procedure=inst)
            ...
            # BISAError shouldn't really happen though
            l.debug("Caught an BISAError during CFG recovery at %#x (%s)", addr, sec_name, exc_info=True)
    ...
    def _detect_loops(self, loop_callback=None):
        ...
        if loop_callback is not None:
            ...
            loop: bisa.analyses.loopfinder.Loop
    ...
    def _immediate_dominators(self, node, target_graph=None, reverse_graph=False):
        ...
        if node not in target_graph:
            raise BISACFGError(f"Target node {node} is not in graph.")
    ...
    def _get_nx_paths(self, begin, end):
        ...
        if isinstance(begin, int) and isinstance(end, int):
            ...
            raise BISACFGError("from and to should be of the same type")
