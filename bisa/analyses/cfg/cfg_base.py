...
from bisa.knowledge_plugins.functions.function_manager import FunctionManager
from bisa.knowledge_plugins.functions.function import Function
from bisa.knowledge_plugins.cfg import IndirectJump, CFGNode, CFGENode, CFGModel  # pylint:disable=unused-import
from bisa.procedures.stubs.UnresolvableJumpTarget import UnresolvableJumpTarget
from bisa.utils.constants import DEFAULT_STATEMENT
from bisa.procedures.procedure_dict import SIM_PROCEDURES
from bisa.errors import (
    ...
    SimIRSBError,
    SimEngineError,
    BISAUnsupportedSyscallError,
    SimError,
)
from bisa.codenode import HookNode, BlockNode
from bisa.engines.vex.lifter import VEX_IRSB_MAX_SIZE, VEX_IRSB_MAX_INST
from bisa.analyses import Analysis
from bisa.analyses.stack_pointer_tracker import StackPointerTracker
from bisa.utils.orderedset import OrderedSet
...
class CFGBase(Analysis):
    ...
    def __init__(
        ...
        :param bisa.SimState base_state:            A state to use as a backer for all memory loads.
        ...
        for start, end in regions:
            if end < start:
                raise BISACFGError("Invalid region bounds (end precedes start)")
    ...
    def functions(self):
        ...
        :rtype: bisa.knowledge_plugins.FunctionManager
        """
    ...
    def _to_snippet(self, cfg_node=None, addr=None, size=None, thumb=False, jumpkind=None, base_state=None):
        ...
        :param bisa.analyses.CFGNode cfg_node: The CFGNode instance.
    ...
    def _determine_function_returning(self, func, all_funcs_completed=False):
        ...
        if self.project.is_hooked(func.addr):
            ...
            except BISAUnsupportedSyscallError:
                procedure = None
    ...
    def _process_irrational_functions(self, functions, predetermined_function_addrs, blockaddr_to_function):
        """
        When force_complete_scan is enabled, for unresolveable indirect jumps, bisa will find jump targets and mark
        ...
        :param bisa.knowledge_plugins.FunctionManager functions: all functions that bisa recovers, including those ones
    ...
    def _process_irrational_function_starts(self, functions, predetermined_function_addrs, blockaddr_to_function):
        ...
        :param FunctionManager functions:   All functions that bisa recovers.
        :return:                            A set of addresses of all removed functions.
        :rtype:                             set
        """
    ...
    def _process_jump_table_targeted_functions(
        ...
        conflicts with how bisa handles jumps to these functions (because they weren't functions to start with). Here
        ...
        usually not a problem until we need to decompile this function, where (at least for now) bisa decompiler
    ...
    def _addr_to_function(self, addr, blockaddr_to_function, known_functions):
        ...
        :param bisa.knowledge_plugins.FunctionManager known_functions: Recovered functions.
        :return: a Function object
        :rtype: bisa.knowledge.Function
        """
    ...
    def _graph_bfs_custom(self, g, starts, callback, blockaddr_to_function, known_functions, traversed_cfg_nodes=None):
        ...
        :param dict blockaddr_to_function: A mapping between block addresses to Function instances.
        :param bisa.knowledge_plugins.FunctionManager known_functions: Already recovered functions.
