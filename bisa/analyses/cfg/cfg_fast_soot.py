...
from bisa.errors import BISACFGError, SimMemoryError, SimEngineError
from bisa.codenode import HookNode, SootBlockNode
...
class CFGFastSoot(CFGFast):
    def __init__(self, support_jni=False, **kwargs):
        ...
        if self.project.arch.name != "Soot":
            raise BISACFGError("CFGFastSoot only supports analyzing Soot programs.")
        ...
        if entry_func is not None:
            ...
            if method_inst is not None:
                ...
                if method_inst is not None:
                    ...
                    raise BISACFGError("There is no method in the Jar file.")
