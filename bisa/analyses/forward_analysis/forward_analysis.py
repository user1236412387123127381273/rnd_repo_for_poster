...
from bisa.errors import BISAForwardAnalysisError
from bisa.errors import BISASkipJobNotice, BISADelayJobNotice, BISAJobMergingFailureNotice, BISAJobWideningFailureNotice
from bisa.utils.algo import binary_insert
...
class ForwardAnalysis(Generic[AnalysisState, NodeType, JobType, JobKey]):
    ...
    static analyses in bisa, including CFG analysis, VFG analysis, DDG, etc.
    ...
    def __init__(
        ...
        if self._allow_widening and not self._allow_merging:
            raise BISAForwardAnalysisError("Merging must be allowed if widening is allowed.")
    ...
    def _analysis_core_baremetal(self) -> None:
        ...
        while not self.should_abort:
            ...
            except BISADelayJobNotice:
                # delay the handling of this job
                continue
            except BISASkipJobNotice:
            ...
            if key in self._job_map:
                ...
                if self._allow_widening and self._should_widen_jobs(job_info.job, job):
                    try:
                        ...
                        job_added = True
                    except BISAJobWideningFailureNotice:
                        # widening failed
                ...
                if not job_added:
                    ...
                    except BISAJobMergingFailureNotice:
                        # merging failed
