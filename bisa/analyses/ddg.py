...
from bisa.errors import SimSolverModeError, SimUnsatError, BISADDGError
from bisa.sim_variable import (
    SimRegisterVariable,
    SimMemoryVariable,
...
class LiveDefinitions:
    ...
    def branch(self):
        ...
        :rtype: bisa.analyses.ddg.LiveDefinitions
        """
...
class DDG(Analysis):
    ...
    Analysis), and then construct a dependence graph on top of the analysis result (for example, the VFG in bisa).
    ...
    The DDG is based on a CFG, which should ideally be a CFGEmulated generated with the following options:
      ...
      - state_add_options=bisa.options.refs to store memory, register, and temporary value accesses
    ...
    def __init__(self, cfg, start=None, call_depth=None, block_addrs=None):
        ...
        :param cfg:         Control flow graph. Please make sure each node has an associated `state` with it, e.g. by
                            passing the keep_state=True and state_add_options=bisa.options.refs arguments to
                            CFGEmulated.
        ...
        if not cfg._keep_state:
            raise BISADDGError('CFG must have "keep_state" set to True.')
    ...
    def _def_lookup(self, variable):  # pylint:disable=no-self-use
        ...
        :param bisa.analyses.ddg.LiveDefinitions live_defs:
        ...
        for code_loc in self._live_defs.lookup_defs(variable):
            ...
            if isinstance(variable, SimMemoryVariable):
                ...
                raise BISADDGError(f"Unknown variable type {type(variable)}")
