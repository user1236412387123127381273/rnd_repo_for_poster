...
from bisa import BP_BEFORE, BP_AFTER, sim_options
from bisa.errors import BISATracerError, SimIRSBNoDecodeError
...
class TracerDesyncError(BISATracerError):
    """
...
class RepHook:
    ...
    def run(self, state):
        from bisa import SIM_PROCEDURES  # pylint: disable=import-outside-toplevel
...
class Tracer(ExplorationTechnique):
    """
    An exploration technique that follows an bisa path with a concrete input.
    ...
    :param resiliency:          Should we continue to step forward even if qemu and bisa disagree?
    ...
    def _locate_entry_point(self, bisa_addr):
        # ...via heuristics
        indices = set()
        threshold = 0x40000
        while not indices and threshold > 0x2000:
            for idx, addr in enumerate(self._trace):
                if ((addr - bisa_addr) & 0xFFF) == 0 and (idx == 0 or abs(self._trace[idx - 1] - addr) > threshold):
                    indices.add(idx)

            indices = {i for i in indices if self._filter_idx(bisa_addr, i)}
            threshold //= 2
    ...
    def _identify_aslr_slides(self):
        """
        libraries can be mapped differently in the original run(in the trace) and in bisa
        this function identifies the difference(called aslr slides) of each library to help bisa translate
        original address and address in bisa back and forth
        """
        if self._aslr:
    ...
    def _filter_idx(self, bisa_addr, idx):
        slide = self._trace[idx] - bisa_addr
        block = self.project.factory.block(bisa_addr)
    ...
    def setup(self, simgr):
        ...
        if len(simgr.active) != 1:
            raise BISATracerError("Tracer is being invoked on a SimulationManager without exactly one active state")
        ...
        if self._fast_forward_to_entry:
            ...
            while simgr.one_active.addr != self.project.entry:
                ...
                if len(simgr.active) == 0:
                    raise BISATracerError("Could not step to the first address of the trace - simgr is empty")
                if len(simgr.active) > 1:
                    raise BISATracerError(
    ...
    def _force_resync(self, simgr, state, deviating_trace_idx, deviating_addr, kwargs):
        ...
        # if unicorn engine is enabled, disable it. forced execution requires single-stepping in bisa.
            ...
            for addr_idx, addr in enumerate(state.history.recent_bbl_addrs):
                ...
                if self._compare_addr(self._trace[idx], addr) or self._check_qemu_unicorn_large_block_split(
                    ...
                    raise TracerDesyncError(
                        "Oops! bisa did not follow the trace", deviating_addr=addr, deviating_trace_idx=idx
                    )
        ...
        if sync == "entry":
            ...
            if proc.display_name == "LinuxLoader":
                ...
                if not self._sync_callsite(state, idx, state.history.addr):
                    raise BISATracerError("Could not synchronize following vsyscall")
            elif self.project.hooked_by(state.addr).display_name.startswith("IFuncResolver"):
                if not self._sync_return(state, idx):
                    raise BISATracerError("Could not synchronize at ifunc return address")
            else:
                # see above
                pass
            ...
            raise TracerDesyncError(
                "Oops! bisa did not follow the trace", deviating_addr=state.addr, deviating_trace_idx=idx + 1
            )
    ...
    def _compare_addr(self, trace_addr, state_addr):
        ...
        raise BISATracerError(f"Trace desynced on jumping into {state_addr:#x}, where no library is mapped!")
    ...
    def _check_qemu_unicorn_large_block_split(self, state: SimState, trace_curr_idx, state_desync_block_idx):
        ...
        while True:
            ...
            if any(curr_block_last_insn.group(insn_type) for insn_type in control_flow_insn_types):
                # Found last block
                bisa_big_block_end_addr = curr_block.addr + curr_block.size - 1
                break
        ...
        for trace_block_idx in range(trace_curr_idx, len(self._trace)):
            ...
            for insn_type in control_flow_insn_types:
                if trace_block_last_insn.group(insn_type):
                    ...
                    if bisa_big_block_end_addr != big_block_end_addr:
    ...
    def _analyze_misfollow(self, state, idx):
        bisa_addr = state.addr
        obj = self.project.loader.find_object_containing(bisa_addr)
        ...
        l.info("Misfollow: bisa says %#x, trace says %#x", bisa_addr, trace_addr)
        if not obj.contains_addr(trace_addr):
            l.error("Translated trace address lives in a different object from the bisa trace")
            return False
        ...
        if "IRSB" in state.history.recent_description:
            ...
            # Case 1: bisa block contains more instructions than trace block
            if self._trace[idx + 1] - slide in last_block.instruction_addrs:
                # we have disparate block sizes!
                # specifically, the bisa block size is larger than the trace's.
            ...
            # Case 2: trace block contains more instructions than bisa
    ...
    def _fast_forward(self, state):
        ...
        if target_obj not in self._aslr_slides:
            ...
            raise BISATracerError("Trace needs to synchronize at an address for which the ASLR slide is unavailable!")
    ...
    def crash_windup(cls, state, crash_addr):
        ...
        try:
            ...
            # See https://github.com/bisa/bisa/issues/71
