...
from bisa.sim_type import SimType, SimTypePointer, SimTypeChar, SimTypeString, SimTypeReg
from bisa.calling_conventions import default_cc
from bisa.knowledge_base import KnowledgeBase
from bisa.errors import BISADirectorError
...
class BaseGoal:
    ...
    def check(self, cfg, state, peek_blocks):
        ...
        :param bisa.analyses.CFGEmulated cfg:   An instance of CFGEmulated.
        :param bisa.SimState state:             The state to check.
    ...
    def _get_cfg_node(cfg, state):
        """
        Get the CFGNode object on the control flow graph given an bisa state.

        :param bisa.analyses.CFGEmulated cfg:   An instance of CFGEmulated.
        :param bisa.SimState state:             The current state.
            ...
            for arg in self.arguments:
                if arg is not None:
                    ...
                    if not isinstance(arg_type, SimType):
                        raise BISADirectorError("Each argument type must be an instance of SimType.")

                    if isinstance(expected_value, claripy.ast.Base) and expected_value.symbolic:
                        raise BISADirectorError("Symbolic arguments are not supported.")
