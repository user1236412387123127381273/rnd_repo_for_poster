...
from bisa.errors import BISAMissingTypeError, BISATypeError
...
class SimTypeArray(SimType):
    ...
    def store(self, state, addr, value: list[StoreType]):
        if self.elem_type.size is None:
            raise BISATypeError("Cannot call store on an array of unsized types")
...
class SimStruct(NamedTypeMixin, SimType):
    ...
    def size(self):
        ...
        if last_type.size is None:
            raise BISATypeError("Cannot compute the size of a struct with elements with no size")
    ...
    if isinstance(decl, c_ast.Struct):
        ...
        if decl.name is not None:
            ...
            if struct is not None and not isinstance(struct, SimStruct):
                raise BISATypeError("Provided a non-SimStruct value for a type that must be a struct")
    ...
    if isinstance(decl, c_ast.Union):
        ...
        if decl.name is not None:
            ...
            if union is not None and not isinstance(union, SimUnion):
                raise BISATypeError("Provided a non-SimUnion value for a type that must be a union")
...
def dereference_simtype(
    ...
    if isinstance(t, SimTypeRef):
        ...
        if real_type is None:
            raise BISAMissingTypeError(f"Missing type {t.name}")
