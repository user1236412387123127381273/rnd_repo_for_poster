...
from bisa.codenode import BlockNode, HookNode, SyscallNode
from bisa.engines.successors import SimSuccessors
from bisa.serializable import Serializable
from bisa.protos import cfg_pb2

if TYPE_CHECKING:
    from bisa.block import Block, SootBlock
...
class CFGNode(Serializable):
    ...
    def __init__(
        ...
        self.block_id: bisa.analyses.cfg.cfg_job_base.BlockID | int = block_id
        self.thumb = thumb
    ...
    def block(self) -> Block | SootBlock | None:
        if self.is_simprocedure or self.is_syscall:
            return None
        project = self._cfg_model.project  # everything in bisa is connected with everything...
