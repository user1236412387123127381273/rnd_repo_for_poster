...
from bisa.engines.vex.lifter import VEX_IRSB_MAX_SIZE
from bisa.misc.ux import once
from bisa.protos import cfg_pb2, primitives_pb2
from bisa.serializable import Serializable
from bisa.utils.enums_conv import cfg_jumpkind_to_pb, cfg_jumpkind_from_pb
from bisa.errors import BISACFGError
from .cfg_node import CFGNode
from .memory_data import MemoryData, MemoryDataSort
from .indirect_jump import IndirectJump

if TYPE_CHECKING:
    from bisa.knowledge_base.knowledge_base import KnowledgeBase
    from bisa.knowledge_plugins.xrefs import XRefManager, XRef
...
class CFGModel(Serializable):
    ...
    def get_exit_stmt_idx(self, src_block, dst_block):
        ...
        if not self.graph.has_edge(src_block, dst_block):
            raise BISACFGError(f"Edge ({src_block}, {dst_block}) does not exist in CFG")
    ...
    def get_intersecting_functions(
        ...
        if kb is None:
            if self.project is None:
                raise BISACFGError("Please provide knowledge base")
            kb = self.project.kb
