...
from bisa.knowledge_plugins.cfg.memory_data import MemoryDataSort
from bisa.codenode import CodeNode, BlockNode, HookNode, SyscallNode
from bisa.serializable import Serializable
from bisa.errors import BISAValueError, SimEngineError, SimMemoryError
from bisa.procedures import SIM_LIBRARIES
from bisa.procedures.definitions import SimSyscallLibrary
from bisa.protos import function_pb2
from bisa.calling_conventions import DEFAULT_CC, default_cc
from bisa.sim_type import SimTypeFunction, parse_defns
from bisa.calling_conventions import SimCC
from bisa.project import Project
from bisa.utils.library import get_cpp_function_name
...
class Function(Serializable):
    ...
    def blocks(self):
        ...
        :return: bisa.lifter.Block instances.
        """
    ...
    def xrefs(self):
        ...
        :return: bisa.knowledge_plugins.xrefs.xref.XRef instances.
        """
    ...
    def get_block(self, addr: int, size: int | None = None, byte_string: bytes | None = None):
        ...
        :param int size:    The size of the block. This is optional. If not provided, bisa will load
        :param byte_string:
        :return:
        """
    ...
    def _confirm_fakeret(self, src, dst):
        if src not in self.transition_graph or dst not in self.transition_graph[src]:
            raise BISAValueError(f"FakeRet edge ({src}, {dst}) is not in transition graph.")

        data = self.transition_graph[src][dst]

        if "type" not in data or data["type"] != "fake_return":
            raise BISAValueError(f"Edge ({src}, {dst}) is not a FakeRet edge")
    ...
    def _call_to(self, from_node, to_func, ret_node, stmt_idx=None, ins_addr=None, return_to_outside=False):
        ...
        :type  from_addr:   bisa.knowledge.CodeNode
        ...
        :param ret_node     The basic block that control flow should return to after the
                            function call.
        :type  to_func:     bisa.knowledge.CodeNode or None
    ...
    def find_declaration(self, ignore_binary_name: bool = False, binary_name_hint: str | None = None) -> bool:
        ...
        :param ignore_binary_name:  Do not rely on the executable or library where the function belongs to determine
                                    ...
                                    libraries in bisa to find the first declaration match.
