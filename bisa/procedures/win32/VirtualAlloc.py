...
def convert_prot(prot):
    """
    Convert from a windows memory protection constant to an bisa bitmask
    """
    ...
    if prot & 0x08:
        return 3
    raise bisa.errors.SimValueError(f"Unknown windows memory protection constant: {prot:#x}")


def deconvert_prot(prot):
    """
    Convert from a bisa bitmask to a windows memory protection constant
    """
    if prot in (2, 6):
        raise bisa.errors.SimValueError("Invalid memory protection for windows process")
    return [0x01, 0x02, None, 0x04, 0x10, 0x20, None, 0x40][prot]


# https://msdn.microsoft.com/en-us/library/windows/desktop/aa366890(v=vs.85).aspx
class VirtualAlloc(bisa.SimProcedure):
    def run(self, lpAddress, dwSize, flAllocationType, flProtect):
        ...
        if len(addrs) != 1:
            raise bisa.errors.SimValueError("VirtualAlloc can't handle symbolic lpAddress")
        addr = addrs[0]
        addr &= ~0xFFF
        ...
        if len(flagss) != 1:
            raise bisa.errors.SimValueError("VirtualAlloc can't handle symbolic flAllocationType")
        flags = flagss[0]

        prots = self.state.solver.eval_upto(flProtect, 2)
        if len(prots) != 1:
            raise bisa.errors.SimValueError("VirtualAlloc can't handle symbolic flProtect")
        prot = prots[0]
        bisa_prot = convert_prot(prot)
        ...
        if flags & 0x00002000 or addr == 0:  # MEM_RESERVE
            if addr == 0:
                ...
                while True:
                    ...
                    try:
                        self.state.memory.map_region(addr, size, bisa_prot, init_zero=True)
