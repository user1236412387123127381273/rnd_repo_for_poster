...
class LoadLibraryA(bisa.SimProcedure):
...
class GetProcAddress(bisa.SimProcedure):
    def run(self, lib_handle, name_addr):
        if lib_handle.symbolic:
            raise bisa.errors.SimValueError(f"GetProcAddress called with symbolic library handle {lib_handle}")
