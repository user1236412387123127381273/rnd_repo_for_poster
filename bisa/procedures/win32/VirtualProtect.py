...
class VirtualProtect(bisa.SimProcedure):
    def run(self, lpAddress, dwSize, flNewProtect, lpfOldProtect):
        ...
        if len(addrs) != 1:
            raise bisa.errors.SimValueError("VirtualProtect can't handle symbolic lpAddress")
        addr = addrs[0]
        ...
        if len(prots) != 1:
            raise bisa.errors.SimValueError("VirtualProtect can't handle symbolic flNewProtect")
        prot = prots[0]
        ...
        for page in range(page_start, page_end + 0x1000, 0x1000):
            self.state.memory.permissions(page, bisa_prot)
