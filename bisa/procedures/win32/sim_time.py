...
class GetSystemTimeAsFileTime(bisa.SimProcedure):
    timestamp = None
...
class GetLocalTime(bisa.SimProcedure):
    wYear = None
    wMonth = None
    wDayOfWeek = None
    wDay = None
    ...
    def fill_symbolic(self):
        ...
        self.wMilliseconds = self.state.solver.BVS(
            "cur_millisecond", 16, key=("api", "GetLocalTime", "cur_millisecond")
        )
...
class QueryPerformanceCounter(bisa.SimProcedure):
    def run(self, ptr):
        if bisa.options.USE_SYSTEM_TIMES in self.state.options:
...
class GetTickCount(bisa.SimProcedure):
    def run(self):
        if bisa.options.USE_SYSTEM_TIMES in self.state.options:
...
class GetTickCount64(bisa.SimProcedure):
