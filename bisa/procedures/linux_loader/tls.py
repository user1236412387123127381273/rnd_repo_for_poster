...
import bisa
from bisa.errors import SimValueError


class __tls_get_addr(bisa.SimProcedure):
...
class ___tls_get_addr(bisa.SimProcedure):
    # pylint: disable=arguments-differ
    def run(self):
        if self.state.arch.name == "X86":
            ptr = self.state.regs.eax
            # use SIM_PROCEDURES so name-mangling doesn't fuck us :|
            return self.inline_call(bisa.SIM_PROCEDURES["linux_loader"]["__tls_get_addr"], ptr).ret_expr
        raise bisa.errors.SimUnsupportedError("___tls_get_addr only implemented for x86. Talk to @rhelmot.")


class tlsdesc_resolver(bisa.SimProcedure):
    # pylint: disable=arguments-differ
...
class _dl_get_tls_static_info(bisa.SimProcedure):
    # pylint: disable=arguments-differ
