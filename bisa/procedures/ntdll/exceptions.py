...
class KiUserExceptionDispatcher(bisa.SimProcedure):
    ...
    def run(self, record, context):
        if self.state.arch.name != "X86":
            raise bisa.errors.SimUnsupportedError("KiUserDispatchException is only implemented for X86")
    ...
    def dispatch(self, record, context):
        if self.call_ret_expr is not None:
            try:
                ...
                raise bisa.errors.SimError(f"Exception handler returned symbolic value {self.call_ret_expr}") from err
            ...
            if disposition == 1:  # unhandled, continue search
                ...
                raise bisa.errors.SimUnsupportedError("Exception disposition ExceptionNestedException is unsupported")
            elif disposition == 3:
                raise bisa.errors.SimUnsupportedError("Exception disposition ExceptionCollidedUnwind is unsupported")
            else:
                raise bisa.errors.SimError(f"Bad exception disposition {disposition}")

        # todo: check cur_ptr against stack bounds
        cur_ptr = self.cur_ptr
        if self.state.solver.is_true(cur_ptr == -1):
            raise bisa.errors.SimError("Unhandled exception - exhausted all exception handlers")
