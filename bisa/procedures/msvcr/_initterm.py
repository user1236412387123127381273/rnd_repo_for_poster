...
class _initterm(bisa.SimProcedure):
    ...
    def get_callbacks(self, fp_a, fp_z):
        ...
        for addr in reversed(
            self.state.memory.load(fp_a, table_size, endness=self.state.arch.memory_endness).chop(self.state.arch.bits)
        ):
