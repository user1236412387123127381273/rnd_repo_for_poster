...
import claripy
import logging
from bisa.errors import SimProcedureError
...
class strtol(bisa.SimProcedure):
    @staticmethod
    ...
    def _string_to_int(s, state, region, base, signed, read_length=None):
        ...
        if not cutoff:
            ...
            if read_length is None:
                # the loop broke because we hit bisa's coded max. we need to assert that the following char is not
