...
class accept(bisa.SimProcedure):
        ...
        if not sockfd.symbolic:
            ...
            if sockfd in self.state.posix.fd:
                ...
                for potential_ident in self.state.posix.sockets:
                    if (
                        self.state.posix.sockets[potential_ident][0] is simsockfd.read_storage
                        and self.state.posix.sockets[potential_ident][1] is simsockfd.write_storage
                    ):
