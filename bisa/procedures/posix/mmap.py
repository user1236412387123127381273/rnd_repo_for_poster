...
import bisa
from bisa.storage.file import SimFileDescriptor
    ...
    def run(self, addr, length, prot, flags, fd, offset):  # pylint:disable=arguments-differ,unused-argument
        ...
        if self.state.solver.is_false(fd[31:0] == -1):
            if self.state.solver.symbolic(fd):
                raise bisa.errors.SimPosixError("Can't map a symbolic file descriptor!!")
            if self.state.solver.symbolic(offset):
                raise bisa.errors.SimPosixError("Can't map with a symbolic offset!!")
        ...
        if sim_fd:
            if not sim_fd.file.seekable:
                raise bisa.errors.SimPosixError("Only support seekable SimFile at the moment.")
