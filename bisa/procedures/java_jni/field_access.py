...
from bisa.engines.soot.exceptions import SootFieldNotLoadedException
from bisa.engines.soot.field_dispatcher import resolve_field
from bisa.engines.soot.values import SimSootValue_InstanceFieldRef, SimSootValue_StaticFieldRef

l = logging.getLogger("bisa.procedures.java_jni.field_access")
...
class GetFieldID(JNISimProcedure):
    ...
    def run(self, ptr_env, field_class_, ptr_field_name, ptr_field_sig):
        ...
        # resolve field
        try:
            field_id = resolve_field(self.state, field_class, field_name, field_type, raise_exception_if_not_found=True)
...
class GetStaticField(JNISimProcedure):
    def run(self, ptr_env, _, field_id_):
        ...
        field_ref = SimSootValue_StaticFieldRef(
            class_name=field_id.class_name, field_name=field_id.name, type_=field_id.type
        )
...
class GetField(JNISimProcedure):
    def run(self, ptr_env, obj_, field_id_):
        ...
        field_id = self.state.jni_references.lookup(field_id_)
        field_ref = SimSootValue_InstanceFieldRef(
            heap_alloc_id=obj.heap_alloc_id,
