...
from bisa.sim_type import parse_cpp_file, SimTypeFunction, SimTypeBottom
from bisa.calling_conventions import DEFAULT_CC
from bisa.misc import autoimport
from bisa.misc.ux import once
from bisa.sim_type import parse_file
from bisa.procedures.stubs.ReturnUnconstrained import ReturnUnconstrained
from bisa.procedures.stubs.syscall_stub import syscall as stub_syscall

if TYPE_CHECKING:
    from bisa.calling_conventions import SimCCSyscall
...
class SimTypeCollection:
    ...
    def get(self, name: str, bottom_on_missing: bool = False) -> SimType:
        ...
        if name not in self.types:
            raise BISAMissingTypeError(f"Type {name} is missing")
...
class SimLibrary:
    ...
    Any instance of this class (or its subclasses) found in the ``bisa.procedures.definitions`` package will be
    automatically picked up and added to ``bisa.SIM_LIBRARIES`` via all its names.
...
def load_external_definitions():
    """
    Load library definitions from specific directories. By default it parses BISA_EXTERNAL_DEFINITIONS_DIRS as a
    semicolon separated list of directory paths. Then it loads all .py files in each directory. These .py files should
    declare SimLibrary() objects and call .set_library_names() to register themselves in bisa.SIM_LIBRARIES.
    """

    global _EXTERNAL_DEFINITIONS_DIRS

    if _EXTERNAL_DEFINITIONS_DIRS is None and "BISA_EXTERNAL_DEFINITIONS_DIRS" in os.environ:
        _EXTERNAL_DEFINITIONS_DIRS = os.environ["BISA_EXTERNAL_DEFINITIONS_DIRS"].strip('"').split(";")
    ...
    if _EXTERNAL_DEFINITIONS_DIRS:
        ...
        # be overwritten by embedded definitions in bisa, which is undesirable
...
def load_all_definitions():
    ...
    if once("load_all_definitions"):
        for _ in autoimport.auto_import_modules("bisa.procedures.definitions", _DEFINITIONS_BASEDIR):
            pass
