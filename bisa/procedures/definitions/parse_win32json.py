...
import bisa
from bisa.sim_type import SimTypeFunction, SimTypeLong
from bisa.utils.library import parsedcprotos2py
from bisa.procedures.definitions import SimTypeCollection
from bisa.errors import BISAMissingTypeError
...
def get_bisa_type_from_name(name):
    if name == "Byte":
        return bisa.types.SimTypeChar(signed=False, label="Byte")
    if name == "SByte":
        return bisa.types.SimTypeChar(signed=True, label="SByte")
    if name == "Char":
        return bisa.types.SimTypeChar(signed=True, label="Char")
    if name == "UInt16":
        return bisa.types.SimTypeShort(signed=False, label="UInt16")
    if name == "Int16":
        return bisa.types.SimTypeShort(signed=True, label="Int16")
    if name == "Int64":
        return bisa.types.SimTypeLongLong(signed=True, label="Int64")
    if name == "UInt32":
        return bisa.types.SimTypeInt(signed=False, label="UInt32")
    if name == "UInt64":
        return bisa.types.SimTypeLongLong(signed=False, label="UInt64")
    if name == "Int32":
        return bisa.types.SimTypeInt(signed=True, label="Int32")
    if name == "Single":
        return bisa.types.SimTypeFloat(size=32)
    if name == "Double":
        return bisa.types.SimTypeFloat(size=64)
    if name == "UIntPtr":
        return bisa.types.SimTypePointer(bisa.types.SimTypeInt(signed=False, label="UInt"), label="UIntPtr")
    if name == "IntPtr":
        return bisa.types.SimTypePointer(bisa.types.SimTypeInt(signed=True, label="Int"), label="IntPtr")
    if name == "Void":
        return bisa.types.SimTypeBottom(label="Void")
    if name == "Boolean":
        return bisa.types.SimTypeBool(label="Boolean")
    if name == "Guid":
        # FIXME
        return bisa.types.SimTypeBottom(label="Guid")
    print(f"Unhandled Native Type: {name}")
    sys.exit(-1)


def get_typeref_from_struct_type(t: bisa.types.SimType) -> bisa.types.SimType:
    if isinstance(t, bisa.types.SimStruct) and t.name and not is_anonymous_struct(t.name):
        # replace it with a SimTypeRef to avoid duplicate definition
        t = bisa.types.SimTypeRef(t.name, bisa.types.SimStruct)
    return t


def handle_json_type(t, create_missing: bool = False):
    if t["Kind"] == "Native":
        return get_bisa_type_from_name(t["Name"])
    if t["Kind"] == "PointerTo":
        pts_to = get_typeref_from_struct_type(handle_json_type(t["Child"], create_missing=create_missing))
        return bisa.types.SimTypePointer(pts_to)
    if t["Kind"] == "Array":
        elem = get_typeref_from_struct_type(handle_json_type(t["Child"], create_missing=create_missing))
        if t["Shape"]:
            return bisa.types.SimTypeFixedSizeArray(elem, length=int(t["Shape"]["Size"]))
        return bisa.types.SimTypePointer(elem)
    if t["Kind"] == "ApiRef":
        try:
            named_type = typelib.get(t["Name"], bottom_on_missing=create_missing)
        except BISAMissingTypeError:
            if t["Name"] in known_struct_names:
                return bisa.types.SimTypeRef(t["Name"], bisa.types.SimStruct)
            raise
    ...
    if t["Kind"] == "Struct":
        ...
        return bisa.types.SimStruct(fields, name=t["Name"])
    if t["Kind"] == "LPArray":
        pts_to = get_typeref_from_struct_type(handle_json_type(t["Child"], create_missing=create_missing))
        return bisa.types.SimTypePointer(pts_to, label="LPArray")
    if t["Kind"] == "Union":
        for nested_type in t["NestedTypes"]:
            typelib.add(nested_type["Name"], handle_json_type(nested_type, create_missing=create_missing))
        members = {}
        ...
        return bisa.types.SimUnion(members)
    if t["Kind"] == "MissingClrType":
        return bisa.types.SimTypeBottom(label="MissingClrType")
    print(f"Unhandled type: {t}")
    sys.exit(0)


def create_bisa_type_from_json(t):
    if t["Kind"] == "NativeTypedef":
        new_typedef = handle_json_type(t["Def"])
        typelib.add(t["Name"], new_typedef)
    elif t["Kind"] == "Enum":
        # TODO: Handle Enums
        ty = bisa.types.SimTypeInt(signed=False, label=t["Name"])
        ...
        typelib.add(
            t["Name"], bisa.types.SimTypePointer(bisa.types.SimTypeFunction(args, ret_type, arg_names=arg_names))
        )
    elif t["Kind"] == "Com":
        # TODO: Handle Com
        typelib.add(t["Name"], bisa.types.SimTypeBottom(label=t["Name"]))
    ...
    for namespace in api_namespaces:
        ...
        for f in funcs:
            ...
            for param in f["Params"]:
                ...
                args.append(new_param)
                arg_names.append(param["Name"])
            new_func = bisa.types.SimTypeFunction(args, ret_type, arg_names=arg_names)
...
from bisa.procedures.definitions import SimTypeCollection
from bisa.sim_type import SimTypeFunction, \
