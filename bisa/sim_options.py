...
# This option prevents bisa from doing hundreds of constraint solves to resolve symbolic jump targets
NO_SYMBOLIC_JUMP_RESOLUTION = "NO_SYMBOLIC_JUMP_RESOLUTION"

# This option prevents bisa from doing hundreds of constraint solves when it hits a symbolic syscall
...
# this causes bisa to use SimAbstractMemory for the memory region
...
# This causes bisa to support fully symbolic writes. It is very likely that speed will suffer.
...
# track the history of actions through a path (multiple states). This action affects things on the bisa level
...
# This makes bisa downsize solvers wherever reasonable.
...
# Synchronize memory mapping reported by bisa with the concrete process.
