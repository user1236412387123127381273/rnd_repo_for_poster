...
from bisa import errors
from bisa.errors import SimError, SimCCallError
from bisa.sim_options import USE_SIMPLIFIED_CCALLS
from bisa.state_plugins.sim_action_object import _raw_ast, SimActionObject
...
def pc_calculate_condition_simple(state, cond, cc_op, cc_dep1, cc_dep2, cc_ndep, platform=None):
    ...
    if cc_op == data[platform]["OpTypes"]["G_CC_OP_NUMBER"]:
        raise SimCCallError(
            "G_CC_OP_NUMBER is not supported in pc_calculate_condition_simple(). Consider implementing."
        )
...
def pc_calculate_rdata_c(state, cc_op, cc_dep1, cc_dep2, cc_ndep, platform=None):
    ...
    if cc_op in (
        ...
        data[platform]["OpTypes"]["G_CC_OP_LOGICB"],
    ):
...
### AMD64-specific ones ###
###########################
# https://github.com/bisa/vex/blob/master/priv/guest_amd64_helpers.c#L2272
...
# https://github.com/bisa/vex/blob/master/priv/guest_amd64_helpers.c#L2304
def amd64g_create_mxcsr(state, sseround):
    return 0x1F80 | ((sseround & 3) << 13)


# https://github.com/bisa/vex/blob/master/priv/guest_amd64_helpers.c#L2316
...
# https://github.com/bisa/vex/blob/master/priv/guest_amd64_helpers.c#L2342
...
def x86g_use_seg_selector(state, ldt, gdt, seg_selector, virtual_addr):
    ...
    if (
        ...
        return bad(
            "bisa doesn't support Windows Heaven's gate calls "
...
def armg_calculate_flag_c(state, cc_op, cc_dep1, cc_dep2, cc_dep3):
    ...
    if concrete_op == ARMG_CC_OP_COPY:
        ...
        res = cc_dep1 + cc_dep2 + cc_dep3
        flag = claripy.If(
            cc_dep3 != 0, boolean_extend(claripy.ULE, res, cc_dep1, 32), boolean_extend(claripy.ULT, res, cc_dep1, 32)
        )
    elif concrete_op == ARMG_CC_OP_SBB:
        flag = claripy.If(
            cc_dep3 != 0,
            boolean_extend(claripy.UGE, cc_dep1, cc_dep2, 32),
...
def armg_calculate_flags_nzcv(state, cc_op, cc_dep1, cc_dep2, cc_dep3):
    ...
    vec = [
        (ARMG_CC_SHIFT_N, claripy.Extract(0, 0, n)),
...
def arm64g_calculate_flags_nzcv(state, cc_op, cc_dep1, cc_dep2, cc_dep3):
    ...
    vec = [
        (ARM64G_CC_SHIFT_N, claripy.Extract(0, 0, n)),
