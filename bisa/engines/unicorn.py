...
import bisa
from bisa.errors import SimIRSBError, SimIRSBNoDecodeError, SimValueError
...
from bisa.state_plugins.unicorn_engine import STOP, _UC_NATIVE, unicorn as uc_module
    ...
    def __check(self, num_inst=None, **kwargs):  # pylint: disable=unused-argument
        ...
        if uc_module is None or _UC_NATIVE is None:
            if once("unicorn_install_warning"):
                l.error(
                    "You are attempting to use unicorn engine support even though it or the bisa native layer "
                    "isn't installed"
                )
            return False
