...
from bisa.calling_conventions import SYSCALL_CC
...
from bisa.state_plugins.inspect import BP_AFTER, BP_BEFORE
from bisa.state_plugins.sim_action_object import _raw_ast
from bisa.storage import DUMMY_SYMBOLIC_READ_VALUE
...
class SimSuccessors:
    ...
    def _categorize_successor(self, state):
        ...
        if o.APPROXIMATE_GUARDS in state.options and state.solver.is_false(state.scratch.guard, exact=False):
            ...
            if "ip_at_syscall" in state.arch.registers:
                ...
                # of syscall SimProcedures by syscall handling code in bisa
            ...
            try:
                ...
                if concrete_syscall_nums is not None:
                    ...
                    # However, we still put it to the flat_successors list, and bisa.SimOS.handle_syscall will pick it
                    # up, and create a "unknown syscall" stub for it.
                    self._fix_syscall_ip(state)
                    self.flat_successors.append(state)
            except (BISAUnsupportedSyscallError, BISASyscallError):
                self.unsat_successors.append(state)
    ...
    def _resolve_syscall(state):
        ...
        if len(possible) == 0:
            raise BISAUnsupportedSyscallError("Unsatisfiable state attempting to do a syscall")
...
class SuccessorsEngine(SimEngine[HeavyState, SimSuccessors]):
    ...
    def __init__(self, project: bisa.Project):
