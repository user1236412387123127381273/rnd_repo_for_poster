...
from bisa.engines.engine import SimEngine
from bisa.utils.constants import DEFAULT_STATEMENT
from .lifter import IRSB
from .behavior import OpBehavior
from bisa.errors import BISAError
from bisa.state_plugins.inspect import BP_BEFORE, BP_AFTER
import contextlib
...
class PcodeEmulatorMixin(SimEngine):
    ...
    def _set_value(self, varnode: Varnode, value: BV) -> None:
        ...
        if space.name == "register":
            ...
            raise BISAError(f"Attempted write to unhandled address space '{space.name}'")
    ...
    def _get_value(self, varnode: Varnode) -> BV:
        ...
        raise BISAError(f"Attempted read from unhandled address space '{space_name}'")
    ...
    def _execute_load(self) -> None:
        ...
        if space.name.lower() in ("ram", "mem"):
            ...
            raise BISAError("Load from unhandled address space")
    ...
    def _execute_store(self) -> None:
        ...
        if space.name.lower() in ("ram", "mem"):
            ...
            raise BISAError("Store to unhandled address space")
    ...
    def _execute_callother(self) -> None:  # pylint:disable=no-self-use
        raise BISAError("CALLOTHER emulation not currently supported")

    def _execute_multiequal(self) -> None:  # pylint:disable=no-self-use
        raise BISAError("MULTIEQUAL appearing in unheritaged code?")

    def _execute_indirect(self) -> None:  # pylint:disable=no-self-use
        raise BISAError("INDIRECT appearing in unheritaged code?")

    def _execute_segment_op(self) -> None:  # pylint:disable=no-self-use
        raise BISAError("SEGMENTOP emulation not currently supported")

    def _execute_cpool_ref(self) -> None:  # pylint:disable=no-self-use
        raise BISAError("Cannot currently emulate cpool operator")

    def _execute_new(self) -> None:  # pylint:disable=no-self-use
        raise BISAError("Cannot currently emulate new operator")
