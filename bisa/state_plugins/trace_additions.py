...
class FormatInfoStrToInt(FormatInfo):
    ...
    def compute(self, state):
        self.input_val = bisa.calling_conventions.SimCCCdecl(state.arch).arg(state, self.str_arg_num)
        if self.base_arg is not None:
            self.input_base = state.solver.eval(
                bisa.calling_conventions.SimCCCdecl(state.arch).arg(state, self.base_arg)
            )
...
class FormatInfoIntToStr(FormatInfo):
    ...
    def compute(self, state):
        self.input_val = bisa.calling_conventions.SimCCCdecl(state.arch).arg(state, self.int_arg_num)
        if self.base_arg is not None:
            self.input_base = state.solver.eval(
                bisa.calling_conventions.SimCCCdecl(state.arch).arg(state, self.base_arg)
            )
            ...
            self.input_base = self.base
        self.str_dst_addr = bisa.calling_conventions.SimCCCdecl(state.arch).arg(state, self.str_dst_num)
...
def generic_info_hook(state):
    ...
    if format_info.get_type() == "DontConstrain":
        ...
        arg = bisa.calling_conventions.SimCCCdecl(state.arch).arg(state, arg_num)
    ...
    if chall_resp_plugin.pending_info is not None:
        chall_resp_plugin.backup_pending_info.append(
            (chall_resp_plugin.ret_addr_to_unhook, chall_resp_plugin.pending_info)
        )
        # undo the stops
...
def constraint_hook(state):
    ...
    chall_resp_plugin = state.get_plugin("chall_resp_info")
    if chall_resp_plugin.pending_info is not None and bisa.options.REPLACEMENT_SOLVER in state.options:
        state.inspect.added_constraints = []


class ChallRespInfo(bisa.state_plugins.SimStatePlugin):
    """
    This state plugin keeps track of the reads and writes to symbolic addresses
    """

    def __init__(self):
        bisa.state_plugins.SimStatePlugin.__init__(self)
    ...
    @bisa.state_plugins.SimStatePlugin.memo
    ...
    def merge(self, others, merge_conditions, common_ancestor=None):  # pylint: disable=unused-argument
        raise bisa.errors.SimMergeError("Can't merge ChallRespInfo - what on earth are you doing?")

    def widen(self, others):  # pylint: disable=unused-argument
        raise bisa.errors.SimMergeError("Can't widen ChallRespInfo - what on earth are you doing?")
    ...
    def prep_tracer(state, format_infos=None):
        ...
        state.inspect.b("exit", bisa.BP_BEFORE, action=exit_hook)
        state.inspect.b("syscall", bisa.BP_AFTER, action=syscall_hook)
        state.inspect.b("constraints", bisa.BP_BEFORE, action=constraint_hook)
...
class ZenPlugin(bisa.state_plugins.SimStatePlugin):
    ...
    def merge(self, others, merge_conditions, common_ancestor=None):  # pylint: disable=unused-argument
        raise bisa.errors.SimMergeError("Can't merge ZenPlugin - what on earth are you doing?")

    def widen(self, others):  # pylint: disable=unused-argument
        raise bisa.errors.SimMergeError("Can't widen ZenPlugin - what on earth are you doing?")
    ...
    def prep_tracer(state):
        ...
        state.inspect.b("reg_write", bisa.BP_BEFORE, action=zen_register_write)
        state.inspect.b("mem_write", bisa.BP_BEFORE, action=zen_memory_write)
