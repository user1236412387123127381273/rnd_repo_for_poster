...
from bisa.state_plugins.plugin import SimStatePlugin
from .heap_freelist import SimHeapFreelist, Chunk
from .utils import concretize
from bisa.errors import SimHeapError, SimMergeError, SimSolverError


l = logging.getLogger("bisa.state_plugins.heap.heap_ptmalloc")
    ...
    **NOTE:** The plugin must be registered using ``register_plugin`` with name ``heap`` in order to function properly.
    ...
    def realloc(self, ptr, size):
        ...
        if new_size > old_size:
            ...
            return new_data_ptr
        if new_size < old_size and old_size - new_size >= self._chunk_min_size:
