...
from bisa.sim_type import ALL_TYPES, SimTypeNum
...
if TYPE_CHECKING:
    from bisa.state_plugins.view import SimMemView
...
class SimDebugVariable:
    ...
    def mem(self) -> SimMemView:
        ...
        if name in ALL_TYPES:
            ...
            # FIXME A lot more types are supported by bisa that are not in ALL_TYPES (structs, arrays, pointers)
            # Use a fallback type
...
class SimDebugVariablePlugin(SimStatePlugin):
    ...
    Example:
        >>> p = bisa.Project("various_variables", load_debug_info=True)
