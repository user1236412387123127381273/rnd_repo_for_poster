...
from bisa import sim_options as options
from bisa.engines.vex.claripy.irop import operations as irop_ops
from bisa.errors import SimMemoryError, SimSegfaultError, SimUnicornError, SimUnicornUnsupport, SimValueError
...
def _load_native():
    if sys.platform == "darwin":
        libfile = "bisa_native.dylib"
    elif sys.platform in {"win32", "cygwin"}:
        libfile = "bisa_native.dll"
    else:
        libfile = "bisa_native.so"

    try:
        bisa_path = str(importlib.resources.files("bisa") / "lib" / libfile)
        h = ctypes.CDLL(bisa_path)
...
class Unicorn(SimStatePlugin):
    ...
    def __init__(
        ...
        """
        Initializes the Unicorn plugin for bisa. This plugin handles communication with
        UnicornEngine.
        """
