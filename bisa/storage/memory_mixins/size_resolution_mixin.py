...
from bisa.errors import SimMemoryLimitError, SimMemoryError, SimUnsatError
...
class SizeConcretizationMixin(MemoryMixin):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._concretize_symbolic_write_size = (
            concretize_symbolic_write_size  # in place of the state option CONCRETIZE_SYMBOLIC_WRITE_SIZES
        )
    ...
    def store(self, addr, data, size=None, *, condition=None, **kwargs):
        ...
        try:
            if self._raise_memory_limit_error:
                conc_sizes = list(
                    self.state.solver.eval_upto(size, self._size_limit, extra_constraints=(size <= max_size,))
                )
        ...
        if size.symbolic:
            if any(cs > self._max_symbolic_size for cs in conc_sizes):
                l.warning(
                    "At least one concretized size is over the limit of %d bytes. Constrain them to the limit.",
