...
import bisa
from bisa.errors import BISACallableError, BISACallableMultistateError, BISASimOSError
from bisa.sim_state import SimState
from bisa.state_plugins import SimSystemPosix
...
from bisa.storage.file import SimFileStream, SimFileBase

if TYPE_CHECKING:
    from bisa.sim_procedure import SimProcedure
...
class SimOS:
    ...
    def __init__(self, project: bisa.Project, name: str | None = None):
        ...
        for reg, val, is_addr, mem_region in state.arch.default_register_values:
            ...
            if is_addr:
                if isinstance(mem_region, tuple):
                    ...
                    raise BISASimOSError(f'You must specify the base address for memory region "{mem_region}". ')
    ...
    def state_call(self, addr, *args, **kwargs):
        ...
        prototype = bisa.calling_conventions.SimCC.guess_prototype(args, kwargs.pop("prototype", None)).with_arch(
            self.arch
        )
    ...
    def syscall_cc(self, state: SimState) -> bisa.calling_conventions.SimCCSyscall | None:
