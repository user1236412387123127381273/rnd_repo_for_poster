...
from bisa.errors import BISASimOSError, SimSegfaultException, SimUnsupportedError, SimZeroDivisionException
from bisa import sim_options as o
from bisa.tablespecs import StringTableSpec
from bisa.procedures import SIM_LIBRARIES as L
from bisa.procedures.definitions import load_win32api_definitions
...
class SimWindows(SimOS):
        ...
        for arg in args:
            ...
            if type(arg) is bytes:
                if b'"' in arg or b"\0" in arg:
                    raise BISASimOSError("Can't handle windows args with quotes or nulls in them")
                arg = claripy.BVV(arg)
    ...
    def initialize_segment_register_x64(self, state, concrete_target):
        """
        Set the gs register in the bisa to the value of the fs register in the concrete process
