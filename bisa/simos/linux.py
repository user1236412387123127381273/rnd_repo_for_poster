...
from bisa.procedures import SIM_PROCEDURES as P, SIM_LIBRARIES as L
from bisa.state_plugins import SimFilesystem, SimHostFilesystem
from bisa.storage.file import SimFile, SimFileBase
from bisa.errors import BISASyscallError
...
class SimLinux(SimUserland):
    ...
    def syscall_abi(self, state):
        ...
        if jk == "Ijk_Sys_syscall":
            return "amd64"
        raise BISASyscallError(f"Unknown syscall jumpkind {jk}")
    ...
    def initialize_segment_register_x64(self, state, concrete_target):
        """
        Set the fs register in the bisa to the value of the fs register in the concrete process
