...
from bisa.calling_conventions import SYSCALL_CC, SimCCSyscall
from bisa.errors import BISAUnsupportedSyscallError, SimSolverError
        ...
        try:
            ...
            if allow_unsupported:
                ...
                if not state.solver.satisfiable():
                    raise BISAUnsupportedSyscallError("The program state is not satisfiable") from err
                raise BISAUnsupportedSyscallError("Got a symbolic syscall number") from err
    ...
    def syscall_from_number(self, number, allow_unsupported=True, abi=None):
        ...
        if self.syscall_library is None:
            if not allow_unsupported:
                raise BISAUnsupportedSyscallError(f"{self.name} does not have a library of syscalls implemented")
            proc = P["stubs"]["syscall"]()
        elif not allow_unsupported and not self.syscall_library.has_implementation(number, self.arch, abilist):
            raise BISAUnsupportedSyscallError(f"No implementation for syscall {number}")
        else:
