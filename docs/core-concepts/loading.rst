...
Previously, you saw just the barest taste of bisa's loading facilities - you
...
We briefly mentioned bisa's binary loading component, CLE. CLE stands for "CLE
...
.. code-block:: python
   ...
   # Here's the "externs object", which we use to provide addresses for unresolved imports and bisa internals
...
If you are loading something with ``bisa.Project`` and you want to pass an
...
.. code-block:: python
   ...
   >>> bisa.Project('examples/fauxware/fauxware', main_opts={'backend': 'blob', 'arch': 'i386'}, lib_opts={'libc.so.6': {'backend': 'elf'}})
...
<https://github.com/bisa/bisa/tree/master/bisa/procedures>`_ as SimProcedures.
These builtin procedures are available in the ``bisa.SIM_PROCEDURES``
...
inaccuracies <Gotchas when using bisa>`.
...
* if ``use_sim_procedures`` (this is a parameter to ``bisa.Project``, not
...
* you may specify specific symbols to exclude from being replaced with
  SimProcedures with the parameters to ``bisa.Project``:
  ``exclude_sim_procedures_list`` and ``exclude_sim_procedures_func``.
* Look at the code for ``bisa.Project._register_object`` for the exact
  algorithm.

Hooking
~~~~~~~

The mechanism by which bisa replaces library code with a Python summary is
...
.. code-block:: python
   ...
   >>> stub_func = bisa.SIM_PROCEDURES['stubs']['ReturnUnconstrained'] # this is a CLASS
