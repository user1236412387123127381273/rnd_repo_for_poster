...
To get started with bisa, you'll need to have a basic overview of some
...
Your first action with bisa will always be to load a binary into a *project*.
...
A project is your control base in bisa. With it, you will be able to dispatch
...
.. tip::
   Using and exploring bisa in IPython (or other Python command line
...
* *arch* is an instance of an ``archinfo.Arch`` object for whichever
  ...
  `at your leisure
  <https://github.com/bisa/archinfo/blob/master/archinfo/arch_amd64.py>`_. The
  common ones you care about are ``arch.bits``, ``arch.bytes`` (that one is a
  ``@property`` declaration on the `main Arch class
  <https://github.com/bisa/archinfo/blob/master/archinfo/arch.py>`_),
...
There are a lot of classes in bisa, and most of them require a project to be
...
This section will also serve as an introduction to several basic bisa concepts.
Strap in!
...
address. This is an important fact - *bisa analyzes code in units of basic
...
Here's another fact about bisa - the ``Project`` object only represents an
"initialization image" for the program. When you're performing execution with
bisa, you are working with a specific object representing a *simulated program
...
bits, to represent CPU data in bisa. Note that each bitvector has a ``.length``
...
bisa comes pre-packaged with several built-in analyses that you can use to extract some fun kinds of information from a program. Here they are:
...
for :py:mod:`bisa.analyses`. As an extremely brief example: here's how you
...
.. code-block:: python
   ...
   >>> proj = bisa.Project('/bin/true', auto_load_libs=False)
...
Having read this page, you should now be acquainted with several important bisa
