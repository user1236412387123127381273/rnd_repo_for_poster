...
Hooks in bisa are very powerful! You can use them to modify a program's behavior
...
.. code-block:: python
   ...
   >>> from bisa import Project, SimProcedure
...
conventions, so if bisa's autodetected one isn't right, you can fix that.
...
As an example, let's look at the SimProcedure that bisa uses internally to run
all the shared library initializers for a ``full_init_state`` for a linux
program:

.. code-block:: python

   class LinuxLoader(bisa.SimProcedure):
       NO_RET = True
       IS_FUNCTION = True
