...
An analysis can be created by subclassing the ``bisa.Analysis`` class. In this
...
.. code-block:: python
   ...
   >>> bisa.AnalysesHub.register_default('MockAnalysis', MockAnalysis) # register the class with bisa's global analysis list
...
.. code-block:: python
   ...
   >>> class ProjectSummary(bisa.Analysis):
   ...     def __init__(self):
...
.. code-block:: python
   ...
   >>> class ComplexFunctionAnalysis(bisa.Analysis):
   ...     def __init__(self):
...
Have fun with analyses! Once you master the rest of bisa, you can use analyses
