...
general questions about bisa, for those too lazy to read this whole document.
...
Why is it named bisa?
---------------------

The core of bisa's analysis is on VEX IR, and when something is vexing, it makes
you bisay.

How should "bisa" be stylized?
------------------------------
...
How can I get diagnostic information about what bisa is doing?
--------------------------------------------------------------

bisa uses the standard ``logging`` module for logging, with every package and
...
.. code-block:: python
   ...
   import logging
   logging.getLogger('bisa').setLevel('DEBUG')

You may want to use ``INFO`` or whatever else instead. By default, bisa will
enable logging at the ``WARNING`` level.

Each bisa module has its own logger string, usually all the Python modules above
it in the hierarchy, plus itself, joined with dots. For example,
``bisa.analyses.cfg``. Because of the way the Python logging module works, you
can set the verbosity for all submodules in a module by setting a verbosity
level for the parent module. For example,
``logging.getLogger('bisa.analyses').setLevel('INFO')`` will make the CFG, as
...
We had two design goals in bisa that influenced this choice:


#. bisa needed to be able to analyze binaries from multiple architectures. This
...
* REIL seems promising, but there is no standard reference implementation that
  ...
  wanted to avoid.
* BAP was another possibility. When we started work on bisa, BAP only supported
...
* VEX was the only choice that offered an open library and support for many
  ...
  specifically for program analysis, making it very easy to use in bisa.

While bisa uses VEX now, there's no fundamental reason that multiple IRs cannot
be used. There are two parts of bisa, outside of the ``bisa.engines.vex``
package, that are VEX-specific:


* the jump labels (i.e., the ``Ijk_Ret`` for returns, ``Ijk_Call`` for calls,
  and so forth) are VEX enums.
* VEX treats registers as a memory space, and so does bisa. While we provide
...
How do I serialize bisa objects?
--------------------------------
...
* There is very limited implementation support in bisa for the actual primitive
...
the
`https://github.com/bisa/bisa/blob/master/bisa/exploration_techniques/oppologist.py
...
Why is bisa's CFG different from IDA's?
---------------------------------------

Two main reasons:


* IDA does not split basic blocks at function calls. bisa will, because they are
