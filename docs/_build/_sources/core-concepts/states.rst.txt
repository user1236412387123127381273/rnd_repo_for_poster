...
So far, we've only used bisa's simulated program states (``SimState`` objects)
...
:py:class:`bisa.engines.successors.SimSuccessors`. Unlike normal emulation,
...
Why a list, instead of just a single successor state? Well, bisa's process of
...
symbolic bitvector? Somewhere in the depths of bisa, the comparison ``x > 4`` is
...
.. code-block:: python
   ...
   >>> proj = bisa.Project('examples/fauxware/fauxware')
   >>> state = proj.factory.entry_state(stdin=bisa.SimFile)  # ignore that argument for now - we're disabling a more complicated default setup for the sake of education
   >>> while True:
   ...     succ = state.step()
...
The program we emulated took data from standard input, which bisa treats as an
...
Fauxware was the first program bisa's symbolic execution ever successfully
...
* To use the call state, you should call it with ``.call_state(addr, arg1, arg2,
  ...
  PointerWrapper, i.e. ``bisa.PointerWrapper("point to me!")``. The results of
...
* To specify the calling convention used for a function with ``call_state``, you
  can pass a :py:class:`~bisa.calling_conventions.SimCC` instance as the ``cc``
  argument.:raw-html-m2r:`<br>` We try to pick a sane default, but for special
  cases you will need to help bisa out.
...
:py:class:`bisa.factory.BISAObjectFactory`) for more details.
...
offsets defined in `archinfo <https://github.com/bisa/archinfo>`_.

State Options
-------------

There are a lot of little tweaks that can be made to the internals of bisa that
...
adding to a state through ``bisa.options``. The individual options are named
...
.. code-block:: python
   ...
   >>> s.options.add(bisa.options.LAZY_SOLVES)

   # Create a new state with lazy solves enabled
   >>> s = proj.factory.entry_state(add_options={bisa.options.LAZY_SOLVES})

   # Create a new state without simplification options enabled
   >>> s = proj.factory.entry_state(remove_options=bisa.options.simplification)
...
* ``history.events`` is a semantic listing of "interesting events" which
  ...
  code.
* ``history.actions`` is usually empty, but if you add the ``bisa.options.refs``
...
bisa will track the call stack for the emulated program. On every call
...
complete and detailed documentation of how I/O is modeled in bisa.
