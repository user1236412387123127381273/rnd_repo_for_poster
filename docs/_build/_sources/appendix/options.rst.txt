...
These are sets of options, found as ``bisa.options.xxx``.
...
.. list-table::
   ...
   * - ``resilience``
     - Options that harden bisa's emulation against unsupported operations,
   ...
   * - ``refs``
     - Options that cause bisa to keep a log of all the memory, register, and
...
These are individual option objects, found as ``bisa.options.XXX``.
