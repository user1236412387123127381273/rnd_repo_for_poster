...
analyzing. bisa has several features to make this less of a headache.

Working with types
------------------

bisa has a system for representing types. These SimTypes are found in
...
bisa also has a light wrapper around ``pycparser``, which is a C parser.
...
.. code-block:: python
   ...
   >>> bisa.types.parse_defns("int x; typedef struct llist { char* str; struct llist *next; } list_node; list_node *y;")
...
.. code-block:: python
   ...
   >>> bisa.types.register_types(bisa.types.parse_type('struct abcd { int x; int y; }'))
...
to other parts of bisa to specify data types.

Accessing typed data from memory
--------------------------------

Now that you know how bisa's type system works, you can unlock the full power of
the ``state.mem`` interface! Any type that's registered with the types module
can be used to extract data from memory.

.. code-block:: python

   >>> p = bisa.Project('examples/fauxware/fauxware')
...
``bisa.calling_conventions`` module.
